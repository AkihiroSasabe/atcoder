...
use superslice::*;
fn main() {
    // 2024-04-06 22:17-22:40 (23min)
    // 2024-04-09 20:47-21:15 (28min)
    // 51min
    input! {
        n: usize,
    }
    ...
    let subtree_sizes = get_subtree_sizes(0, &graph, &c);

    let mut seen = vec![false; n];
    let mut ans = f0;
    dfs2(&mut ans, f0, 0, &graph, &mut seen, &subtree_sizes, &c);
    println!("{}", ans);

}

fn dfs2(ans: &mut usize, fv: usize, v: usize, graph: &Vec<Vec<usize>>, seen: &mut Vec<bool>, subtree_sizes: & Vec<usize>, c: &Vec<usize>) {
    seen[v] = true;
    *ans = min(*ans, fv);
    ...
    for i in 0..graph[v].len() {
        ...
        let fnv = fv + subtree_sizes[0] - 2 * subtree_sizes[next_v]; 
        dfs2(ans, fnv, next_v, graph, seen, subtree_sizes, c);
    }
}

/// 各頂点の「部分木の大きさのリストを返す関数
/// root_v := 木の根となる頂点
fn get_subtree_sizes(root_v: usize, graph: &Vec<Vec<usize>>, c: &Vec<usize>) -> Vec<usize> {
    // subtree_sizes[v] := 頂点vを根とする、部分木の大きさ(頂点数)
    let n = graph.len();
    let mut subtree_sizes = vec![0; n];
    let mut seen = vec![false; n];
    dfs(root_v, &graph, &mut seen, &mut subtree_sizes, &c);
    return subtree_sizes
}
fn dfs(v: usize, graph: &Vec<Vec<usize>>, seen: &mut Vec<bool>, subtree_sizes: &mut Vec<usize>, c: &Vec<usize>) {
    seen[v] = true;
    for i in 0..graph[v].len() {
        let next_v = graph[v][i];
        if seen[next_v] {continue}
        dfs(next_v, graph, seen, subtree_sizes, c);
