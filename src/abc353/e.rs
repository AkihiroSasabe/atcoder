...
fn main() {
    ...
    // baseの累乗を事前計算
    let num_unique_char = 26; // baseの値は文字の種類数以上なら何でも良い
    for i in 0..n {
        let rollong_hash = RollingHash::new(&s[i], num_unique_char);
        for j in 0..s[i].len() {
            let hash: (usize, usize) = rollong_hash.get_substring_hash(0, j);
            *btree.entry(hash).or_insert(0) += 1;
        }
    }
    let mut ans: usize = 0;
    for (hash, num) in btree {
        ans += num * (num-1) / 2;
    }
    println!("{}", ans);
}
