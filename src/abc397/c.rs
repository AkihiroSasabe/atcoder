...
fn main() {
    ...
    let mut btree0 = BTreeMap::new();
    let mut btree1 = BTreeMap::new();

    btree0.add_value_by_1(a[0]);

    for i in 1..n {
        btree1.add_value_by_1(a[i]);
    }

    let mut ans = btree0.len() + btree1.len();

    for i in 1..n-1 {
        btree0.add_value_by_1(a[i]);
        btree1.subtract_value_by_1(a[i]);
        let temp = btree0.len() + btree1.len();
        ans = max(ans, temp);
    }
    println!("{}", ans);
