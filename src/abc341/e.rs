...
use superslice::*;
fn main() {
    // 2024-02-27 19:50-21:25 (1h35min) 競プロフレンズの回答を見た
    input! {
        n: usize,
        q: usize,
        s: Chars,
    ...
    for i in 0..q {
        ...
        if ti == 1 {
            if li != 0 {
                let left = bit.sum_range(li-1, li-1);
                if left == 0 {
                    bit.add(li-1, 1);
                }
                else if left == 1 {
    ...
    // let sinsu = 1_000_000;
    // let mut heads = vec![]; // heads[i] := iと同じグループの先頭
    // heads[0] = 0;
    // let mut pre = s[0];
    // for i in 1..n { 
    //     if s[i] != pre {
    ...
    // let mut classes = vec![];
    // for i in 0..n {
    //     let class = heads[i] + tails[i] * sinsu;
    //     classes.push(class);
    // }


    // let mut lazy_segment_tree = lazy_segment_tree::new_range_assignment_update_and_range_minimum_query_from_vec(&classes);
    // for i in 0..q {
    //     let ti = t[i];
    //     let li = l[i];
    //     let ri = r[i];
    //     if ti == 1 {
    //         let class_l_out = lazy_segment_tree.range_query(li-1, li-1);
    //         let class_l = lazy_segment_tree.range_query(li, li);
    //         let class_r = lazy_segment_tree.range_query(ri, ri);
    //         let class_r_out = lazy_segment_tree.range_query(ri+1, ri+1);

    //         // 略字の意味 left-start-out, right-end-in
    //         let lo_s = class_l_out % sinsu;
    //         let lo_t = class_l_out / sinsu;
    //         let li_s = class_l % sinsu;
    //         let li_t = class_l / sinsu;

    //         let ro_s = class_r_out % sinsu;
    //         let ro_t = class_r_out / sinsu;
    //         let ri_s = class_r % sinsu;
    //         let ri_t = class_r / sinsu;
    //         if class_l_out == class_l && class_r_out == class_r {
    //             // 元が良いクラスなら、悪いクラスになる。
                
    //             // 左外側の修正
    //             lazy_segment_tree.range_update(lo_s as usize, li-1, lo_s + (li - 1) as isize * sinsu);
    //             // 右外側の修正
    //             lazy_segment_tree.range_update(ri + 1, ro_t as usize, (ri + 1) as isize + ro_t * sinsu);

    //             // 左内側の修正
    //             lazy_segment_tree.range_update(li, min(li_t as usize, ri), li as isize + min(li_t as usize, ri) as isize * sinsu);

    //             // 右内側の修正
    //             lazy_segment_tree.range_update(max(li, ri_s as usize), ri, max(li as isize, ri_s) + ri as isize * sinsu);
    //         }
    //         else if class_l_out != class_l && class_r_out != class_r {
    //             // 左外側の修正
    //             lazy_segment_tree.range_update(lo_s as usize, li-1, lo_s + (li - 1) as isize * sinsu);
    //             // 右外側の修正
    //             lazy_segment_tree.range_update(ri + 1, ro_t as usize, (ri + 1) as isize + ro_t * sinsu);
    ...
    //         lazy_segment_tree.range_update(li, ri, 1);
