...
fn main() {
    ...
    // 友達グラフの出次数
    let mut out_degs_friend = vec![0; n];
    for i in 0..m {
        input!{
            ai: usize,
            bi: usize,
        ...
        uft.unite(ai-1, bi-1);
        out_degs_friend[ai-1] += 1;
        out_degs_friend[bi-1] += 1;
    }
    for i in 0..k {
        input!{
            ci: usize,
        ...
        graph_b[ci-1].push(di-1);
        graph_b[di-1].push(ci-1);
    }
    for i in 0..n {
        // 友達候補数 = 連結成分数 - 自分の分(1) - 友達数(友達グラフの出次数)
        let mut ans = uft.size(i) - 1 - out_degs_friend[i];
        for &nv in graph_b[i].iter() {
            // ブロックしている奴が、連結成分にいたら、除外する
            if uft.issame(i, nv) {
                ans -= 1;
            }
        }
