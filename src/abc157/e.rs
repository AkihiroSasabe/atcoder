...
fn main() {
    ...
    // Mo's とみせかけて、ただの先読みクエリ
    // 67_108_864 = 6*10^7 <= 10^8
    // 遅延セグメント木でorで和を判定すればいいだけ。
    // let init_vector = vec![init_value; n];
    let mut init_vector = vec![];
    for i in 0..n {
        let num = s[i] as isize - 'a' as isize;
        init_vector.push(1 << num);
    }

    // ただの、区間更新・区間和クエリのセグ木ではないことに注意。
    // 区間和の代わりに、区間論理和　に実装を書き換えてある。
    let mut lazy_segment_tree = lazy_segment_tree::new_range_assignment_update_and_range_sum_query_from_vec(&init_vector);

    for i in 0..q {
        if t[i] == 1 {
            // 更新
            let num_i = c[i] as isize - 'a' as isize;
            lazy_segment_tree.range_update(l[i], l[i], 1 << num_i);
        }
        else {
            // クエリ
            let sum_mask = lazy_segment_tree.range_query(l[i], r[i]).value;
            // 足す、ではなく、orに書き換える。
            let ans = sum_mask.count_ones();
            // let mut ans = 0;
            // for j in 0..26 {
            //     if sum_mask & (1 << j) != 0 {
            //         ans += 1;
    ...
    pub fn new_range_assignment_update_and_range_sum_query(list_size: usize) -> LazySegmentTree<SSum, fn(&SSum, &SSum) -> SSum, fn() -> SSum, isize, fn(&isize, &SSum) -> SSum, fn(&isize, &isize) -> isize, fn() -> isize> {
        ...
        fn mapping(&f: &isize ,&x: &SSum) -> SSum {
            if f == id() {
                ...
                // x.value = f * x.size;
                // return x
                return SSum {value: f, size: x.size}
            }
        }
        /// Lazy -> Lazy の更新操作
        /// f: 追加する最新の更新操作
