...
use superslice::*;
fn main() {
    // 2024-04-04 19:38-20:16 (38min)　解法のひらめきに18分、実装に20分
    input! {
        h: usize,
        w: usize,
        m: usize,
    ...
    let mut init_vector_yokomax = vec![0; h];
    let mut init_vector_tatemax = vec![0; w];
    let mut seg_yokomax = lazy_segment_tree::new_range_increment_update_and_range_maximum_query_from_vec(&init_vector_yokomax);
    let mut seg_tatemax = lazy_segment_tree::new_range_increment_update_and_range_maximum_query_from_vec(&init_vector_tatemax);

    let mut btree = BTreeMap::new();
    for i in 0..m {
        let hi = hh[i];
        let wi = ww[i];
        seg_yokomax.range_update(hi, hi, 1);
        seg_tatemax.range_update(wi, wi, 1);
        btree.entry(wi).or_insert(vec![]).push(hi);
    }
    let mut ans = 0;

    for (wi, hi) in btree.iter() {
        // wi を選んだとき
        let mut temp = seg_tatemax.range_query(*wi, *wi);
        for hij in hi {
            // セグ木
            seg_yokomax.range_update(*hij, *hij, -1);
        }
        temp += seg_yokomax.range_query(0, h-1);
        for hij in hi {
            // セグ木を元に戻す
            seg_yokomax.range_update(*hij, *hij, 1);
        }
