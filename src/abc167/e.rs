...
fn main() {
    ...
    // 方針: 隣り合う色の数が、X個のX<=Kの場合について、個別に考えていく。
    // ・X=0のとき
    // 隣り合う色は全部異なるので、
    // 先頭ブロックから順に、色の組み合わせは、
    // M x (M-1) x (M-1) x ... = M x (M-1)^(N-1)
    
    // ・X=1のとき、連続する場所は、(N-1)_C_1 通りあり、N-1箇所の色の塗り方は、
    // M x (M-1) x (M-1) x ... = M x (M-1)^(N-2)
    // であるから、合計、(N-1)_C_1 * M x (M-1)^(N-2)通り

    // ・X=2のとき、連続する場所は、(N-1)_C_2 通りあり、N-2箇所の色の塗り方は、
    // M x (M-1) x (M-1) x ... = M x (M-1)^(N-3)
    // であるから、合計、(N-1)_C_2 * M x (M-1)^(N-3)通り
    
    // こんな感じで、X=Kのときまで、順に求めれば、O(N)で解ける。
    ...
    // temp := M * (M-1)^(N-X-1) 。まずはX=Kのときの、tempから求める。
    let mut temp = 1;
    let mut count = 0;
    for i in 0..n-k {
        if count == 0 {
    ...
    let mut ans = 0;
    for ki in (0..k+1).rev() {
        ans += temp * get_ncr(n-1, ki, &factorial, modulus) % modulus;
        ans %= modulus;
        if count == 0 {
            temp *= m;
        }
...
fn get_ncr(n: usize, r: usize, factorial: &Vec<usize>, modulus: usize) -> usize {
    let bunsi = factorial[n];
    let bunbo = factorial[r] * factorial[n-r] % modulus;

    let ncr = mod_dev(bunsi, bunbo, modulus);
    return ncr
}
