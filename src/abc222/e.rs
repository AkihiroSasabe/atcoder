...
use superslice::*;
fn main() {
    // 2024-01-21 19:52-22:00 (2h8m)
    // 2024-01-24 19:39-20:49 (1h10m)
    // total: 3h18min = 198min
    input! {
        n: usize,
        m: usize,
        k: isize,
    ...
    let mut hashes = vec![BTreeMap::new(); n];
    for i in 0..n-1 {
        input! {
            ui: usize,
            vi: usize
        ...
        let big = max(ui-1, vi-1);
        let small = min(ui-1, vi-1);
        hashes[small].insert(big, 0);
    }

    // a[i]とa[i+1]を最短で繋ぐ経路上に存在する、エッジの通過回数を求める
    let lca = LowestCommonAncestor::new(&graph, a[0]);
    for i in 0..m-1 {
        let mut u = a[i];
        let mut v = a[i+1];
        let ca = lca.query(u, v);

        while u != ca {
            let pu = lca.parent[0][u];
            let big = max(u, pu);
            let small = min(u, pu);
            *hashes[small].get_mut(&big).unwrap() += 1;
            u = pu;
        }
        while v != ca {
            let pv = lca.parent[0][v];
            let big = max(v, pv);
            let small = min(v, pv);
            *hashes[small].get_mut(&big).unwrap() += 1;
            v = pv;
    ...
    let mk = max(n*m, k.abs() as usize);
    // println!("mk = {:?}", mk);

    // 現在のエッジまでで、
    // dp[赤と青の通過回数差x] := 差がx回になっている塗り方の総数
    let mut dp = vec![0; mk*2+1];
    dp[mk] = 1;

    let mut ind: usize = 0;
    // グラフ内の全エッジを全探索
    for v in 0..n {
        for (&nv, &count) in hashes[v].iter() {
            if count == 0 {
                zero_count += 1;
            }
            else {
                let mut new_dp = vec![0; mk*2+1];
                for k_temp in ((-(mk as isize))..(mk as isize+1)) {
                    let pre = (mk as isize + k_temp) as usize;
                    let next_up = (mk as isize + k_temp + count) as usize;
                    let next_down = (mk as isize + k_temp - count);

                    if next_down > 2*(mk as isize) {continue}
                    if next_down >= 0 {
                        let next_down =  next_down as usize;
                        new_dp[next_down] += dp[pre];
                        new_dp[next_down] %= modulus;    
                    }
                    if next_up > 2*mk {continue}
                    new_dp[next_up] += dp[pre];
                    new_dp[next_up] %= modulus;
                }
                swap(&mut dp, &mut new_dp);
            }
        }
    }
    // debug
    ...
    // for i in 0..n {
    //     println!("hash[{i}] = {:?}", hashes[i]);
    // }

    let mut ans: usize = dp[(k + (mk as isize)) as usize];
