...
fn main() {
    ...
    for i in 0..n {
        for j in 0..60 {
            if (1 << j) & a[i] != 0 {
                // a[i]のj桁目が1だったら、j桁目が0の奴の個数分、増える。
                xor_sum += (n - num_1[j]) * ((1 << j) % modulus) % modulus;
                xor_sum %= modulus;
            }
            else {
                // a[i]のj桁目が0だったら、j桁目が1の奴の個数分、増える。
                if num_1[j] == 0 {continue}
                xor_sum += num_1[j] * ((1 << j) % modulus) % modulus;
                xor_sum %= modulus;
            }
        }
    }
    // println!("xor_sum = {:?}", xor_sum);
    let ans = mod_dev(xor_sum, 2, modulus);
    println!("{}", ans);
