...
fn main() {
    ...
    // (1)Tkahashiの最善手：
    // Takahashi の方が近くて、 Aoki の方が遠い点のうち、Aokiからの距離が最も遠い点の1個手前で衝突する
    // (2)Aokiの最善手：
    // 貪欲に Takahashi に近い方に進む。

    // Takahashiだけ考えれば良い。

    // LCA
    let lca = LowestCommonAncestor::new(&graph, vt);

    // 高橋より遠い点で、かつ青木から一番遠い点を全探索
    let mut max_dist_a = 0;
    for i in 0..n {
        let dist_t = lca.get_distance(i, vt);
        let dist_a = lca.get_distance(i, va);
        if dist_t < dist_a {
            if max_dist_a < dist_a {
                max_dist_a = dist_a;
            }
        }
    }

    println!("{}", max_dist_a-1);
