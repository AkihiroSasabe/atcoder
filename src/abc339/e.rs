...
fn main() {
    ...
    // let mut edges = vec![];
    let max_h = *a.iter().max().unwrap();
    
    let mut array = vec![0; max_h + 1];
    let mut lazy_segment_tree = lazy_segment_tree::new_range_assignment_update_and_range_maximum_query_from_vec(&array);
    ...
    dp[0][1] = 1;
    let h0 = a[0];
    lazy_segment_tree.range_update(h0, h0, dp[0][1]);

    for i in 1..n {
        // iを残さないとき i-1がいても、いなくてもいい
        dp[i][0] = max(dp[i-1][0], dp[i-1][1]); //

        // iを残すとき
        let h = a[i];
        let h_min = max(d, a[i]) - d;
        let h_max = a[i] + d;
        let max_len = lazy_segment_tree.range_query(h_min, h_max);
        dp[i][1] = max_len + 1;

        // これまでのhの最大
        let pre_max_len = lazy_segment_tree.range_query(h, h);
        lazy_segment_tree.range_update(h, h, max(dp[i][1], pre_max_len));
    }
