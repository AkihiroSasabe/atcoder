...
fn main() {
    ...
    // 一番低い位置から、順に 区間最大クエリのセグ木  に移動回数を記録してシミュレーションしていく。
    // 注目している位置を pos とすれば、 [pos - r, pos + r] の範囲にあり、かつ 自分よりd番目以前の、一番移動回数が高い場所 + 1 が posの飛び移れる回数の最大値となる。
    // O(NlogN)でいける。
    let mut init_vector = vec![-1; n];
    let mut lazy = lazy_segment_tree::new_range_assignment_update_and_range_maximum_query_from_vec(&init_vector);
    let mut num_move = vec![0; n];
    for i in 0..n {
        let (hi, pos) = h2[i];

        if i >= d {
            // 今よりd低い足場をセグ木に追加して更新。
            let (hi_pre, pos_pre) = h2[i-d];
            lazy.range_update(pos_pre, pos_pre, num_move[pos_pre]);
        }
        
        let q_l = pos.saturating_sub(r);
        let q_r = min(n-1, pos + r);
        let max_move = lazy.range_query(q_l, q_r);

        // println!("i = {i}, hi = {hi}, max_move = {:?}", max_move);

        num_move[pos] = max_move + 1;
    }

    let ans = num_move.iter().max().unwrap();
    println!("{}", ans);
}
