...
fn main() {
    ...
    // 座標圧縮
    let (ranked_array, _sorted_array, _original_to_rank) = rank_array(&a);

    // セグ木
    // 値
    let mut init_vector = vec![0; n];
    let mut lazy_segment_tree_val = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);

    // 個数
    let mut init_vector = vec![0; n];
    let mut lazy_segment_tree_num = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);

    let mut ans = 0;
    for i in (0..n).rev() {
        if ranked_array[i]+1 >= n {}
        else {
            let val = lazy_segment_tree_val.range_query(ranked_array[i]+1, n-1).value;
            let num = lazy_segment_tree_num.range_query(ranked_array[i]+1, n-1).value;
            ans += val - num * a[i] as isize;
        }
        lazy_segment_tree_val.range_update(ranked_array[i], ranked_array[i], a[i] as isize);
        lazy_segment_tree_num.range_update(ranked_array[i], ranked_array[i], 1);
    }
    println!("{}", ans);
