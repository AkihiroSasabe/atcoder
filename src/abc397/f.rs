...
fn main() {
    // 2025-03-16 17:15-17:57 (42min)
    // 2025-03-16 21:23-22:01 (38min)
    // Total: 80 min
    input! {
        n: usize,
        a: [usize; n],
...
fn solve_mine(n: usize, a: Vec<usize>) {
    ...
    // セグ木 i-1とi の上をまたがる ↑ のグラフのエッジの本数。 i-1 と i をまたがるエッジを何本切断できるかが知りたい。
    let mut init_vector = vec![0; n];
    let mut lazy_segment_tree = lazy_segment_tree::new_range_increment_update_and_range_maximum_query_from_vec(&init_vector);
    for i in 0..n {
        if let Some(&v) = btree.get(&a[i]) {
            lazy_segment_tree.range_update(v+1, i, 1);
            graph[v].push(i);
        }
        btree.insert(a[i], i);
    }
    ...
    // 一番左の領域を1つずつ広げながら、3領域の最大種類数を求めていく。
    let mut ans = 0;
    for st in 0..n-2 {
        if graph[st].len() != 0 {
            let nv = graph[st][0];
            lazy_segment_tree.range_update(st+1, nv, -1);
        }
        // 一番左の領域 (0 ~ st) の種類数
        let num0 = head[st];

        // 真ん中の領域 と 一番右の領域 をあわせた領域 (st+1 ~ n-1) の種類数
        let num12 = tail[st+1];
        
        // 真ん中の領域 と 一番右の領域 の2つの領域にまたがって存在する種類数
        let num_cut = lazy_segment_tree.range_query(st+1, n-1) as usize;
        
        // 真ん中の領域の種類数 と 一番右の領域の種類数 の和の最大
        let temp = num0 + num12 + num_cut;
        // println!("(st, num0, num12, num_cut, temp) = {:?}", (st, num0, num12, num_cut, temp));
        ans = max(ans, temp);
    }
    println!("{}", ans);
}
