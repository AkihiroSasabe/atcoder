...
fn main() {
    ...
    for i in 0..n {
        ...
        a.push(ai); // 園児iのレート。a[園児] = レート
        b.push(bi-1); // 園児iの所属。b[園児] = 幼稚園
    }
    let mut c = vec![];
    let mut d = vec![];
    for i in 0..q {
    ...
    // btrees:= Key (rate, 園児) を格納
    let mut btrees = vec![BTreeSet::new(); num_max_school];
    for hito in 0..n {
        let rate = a[hito];
        let pos = b[hito];
        btrees[pos].insert((rate, hito));
    }

    // lazy_segment_tree := 全幼稚園のno1を格納
    let INF = 1 << 60;
    let init_vector = vec![INF; num_max_school];
    let mut lazy_segment_tree = lazy_segment_tree::new_range_assignment_update_and_range_minimum_query_from_vec(&init_vector);
    for i in 0..num_max_school {
        if let Some((rate, hito)) = btrees[i].iter().rev().next() {
            lazy_segment_tree.range_update(i, i, *rate as isize);
        }
    }
    ...
    for i in 0..q {
        ...
        // 移動元を削除
        btrees[pos_s].remove(&(rate, hito));
        if let Some((rate_2nd, _hito_2nd)) = btrees[pos_s].iter().rev().next() {
            lazy_segment_tree.range_update(pos_s, pos_s, *rate_2nd as isize);
        }
        else {
            lazy_segment_tree.range_update(pos_s, pos_s, INF);
        }

        // 移動先に追加
        btrees[pos_t].insert((rate, hito));
        let (rate_t, _hito_t) = btrees[pos_t].iter().rev().next().unwrap();
        lazy_segment_tree.range_update(pos_t, pos_t, *rate_t as isize);
        b[hito] = pos_t;

        let ans = lazy_segment_tree.range_query(0, num_max_school-1);
        println!("{}", ans);
    }
}
