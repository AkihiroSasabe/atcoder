...
fn main() {
    // 2025-05-11 11:00-11:48 (48min, 考察)
    // 2025-05-11 11:48-12:00 (12min, 実装)
    // Total: 60min
    input! {
        n: usize,
        m: usize,
    }
    let mut base_heads = vec![vec![]; 2*n];
    let mut base_tails = vec![vec![]; 2*n];
    for i in 0..m {
        input!{
            ai: Usize1,
            bi: Usize1,
        }
        base_heads[ai].push(bi);
        base_tails[bi].push((ai, bi));
    }

    let mut q_heads = vec![vec![]; 2*n];
    let mut q_tails = vec![vec![]; 2*n];
    ...
    for i in 0..q {
        ...
        q_heads[ci].push((di, i));
        q_tails[di].push((ci, i));
    }

    // ab: 奇数
    // cd: 偶数 query
    let mut init_vector = vec![0; 2*n];

    // lazy[v] := 土台の末尾vの個数を管理
    // lazy2[v] := 土台の先頭vの個数を管理
    let mut lazy = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    let mut lazy2 = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);

    let mut anss = vec![0; q];
    for v in 0..2*n {
        // 土台の先頭に来たら、 
        // lazy に土台の末尾を追加 && lazy2 に土台の先頭を追加
        for &b_tail in base_heads[v].iter() {
            lazy.range_update(b_tail, b_tail, 1);
            lazy2.range_update(v, v, 1);
        }

        // クエリの先頭に来たら、
        // lazy の中からクエリの先頭 ~ 末尾 の間に、土台の末尾の個数をカウント
        for &(q_tail, q_ind) in q_heads[v].iter() {
            let val = lazy.range_query(v, q_tail).value;
            anss[q_ind] += val;
        }

        // 土台の末尾に来たら、
        // lazy から土台の末尾を削除 & lazy2 から土台の先頭を削除
        for &(b_head, b_tail) in base_tails[v].iter() {
            lazy.range_update(b_tail, b_tail, -1);
            lazy2.range_update(b_head, b_head, -1);
        }

        // クエリの末尾に来たら,
        // lazy2 の中から、クエリの先頭 ~ 末尾 の間にいる土台の先頭の個数をカウント
        for &(q_head, q_ind) in q_tails[v].iter() {
            let val = lazy2.range_query(q_head, v).value;
            anss[q_ind] += val;
        }
    }
    for i in 0..q {
