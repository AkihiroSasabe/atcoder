...
fn main() {
    ...
    // let mut init_vector = vec![0; s+1];
    // let mut lazy_segment_tree = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    let mut dp = vec![lazy_segment_tree; max_num + 1];

    dp[0].range_update(0, 0, modint::Modint::new(modulus, 1));

    for num in 0..max_num {
        // 配る
        for ss in 0..=s-3 {
            let src = dp[num].range_query(ss, ss).value;
            if ss+3 > s {continue}
            // println!("(num, ss, src) = {:?}", (num, ss, src));
            dp[num+1].range_update(ss+3, s, src);
        }
    }
    let mut ans = modint::Modint::new(modulus, 0);
    for num in 1..=max_num {
        let src: modint::Modint = dp[num].range_query(s, s).value;
        // println!("(num, src) = {:?}", (num, src));
        // dp[num].print_all_single_queries();
        ans += src;
    }
    println!("{}", ans.value);
...
pub mod modint {
    ...
    impl Modint {
        pub fn new(modulus: isize, value: isize) -> Modint {
