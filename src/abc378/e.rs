...
use rand::Rng;
fn main() {
    // 2024-11-02 21:37-22:40 (1h3m)
    // 2024-11-02 24:00-27:34 (3h34m, 解説AC)
    // 4h37min
    input! {
        n: usize,
        m: isize,
...
// 自力AC (Lを一番外側の括弧とした場合)
// 2024-11-03 17:08-17:39 (31min)
fn solve_mine(n: usize, m: isize, a: Vec<isize>) {
    let mut cum = vec![0; n]; // cum[i] := (a[0] + .. + a[i]) % m 。つまり、(aの累積和 % m)
    let mut cum2 = vec![0; n]; // cum2[i] := cum[0] + ... + cum[i] 。 cumの累積和だが、 m の剰余ではないことに注意
    cum[0] = a[0] % m;
    cum2[0] = cum[0];
    for i in 1..n{
        cum[i] = (cum[i-1] + a[i]) % m;
    ...
    // 遅延評価セグメント木 := i 番目の配列には、値がi である、cum[i] が何個あるか、が格納されている。
    let mut init_vector = vec![0; m as usize];
    let mut lazy = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    let mut ans = 0;
    for l in (0..n).rev() {
        lazy.range_update(cum[l] as usize, cum[l] as usize, 1);
        let mut num = 0;
        if l != 0 && cum[l-1] != 0 {
            num = lazy.range_query(0, cum[l-1]  as usize -1).value;
        }
        let mut diff = 0;
        if l != 0 {
            diff = cum2[l-1] + cum[l-1] * (n-l) as isize;
        }
        ans += num * m + cum2[n-1] - diff;
    }
    println!("{}", ans);
}

// 解説AC (Rを一番外側の括弧とした場合)
fn solve_education(n: usize, m: isize, a: Vec<isize>) {
    let mut init_vector = vec![0; m as usize];
    let mut lazy = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    ...
    for r in 1..n {
        sl += cum[r-1];
        let diff = (r+1) as isize * cum[r] - sl;
        lazy.range_update(cum[r-1] as usize, cum[r-1] as usize, 1);
        let mut num = 0;
        if cum[r]+1 <= m-1 {
            num = lazy.range_query(cum[r] as usize +1, m as usize -1).value;
        }
        ans += diff + num * m;
        // let r = a[i] % m;
        // lazy.range_update(n-1-i ,n-1, r as isize);
    }
    println!("{}", ans);
}
