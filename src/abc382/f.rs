...
use rand::Rng;
fn main() {
    // 2024-12-01 13:17-13:23 (6 min)
    // 2024-12-01 14:12-14:35 (23min)
    // 29min
    input! {
        h: usize,
        w: usize,
    ...
    for i in 0..n {
        ...
        cs.push(csi);
        ls.push(lsi);
        rcls.push((rsi, csi, lsi, i));
    }
    rcls.sort();
    rcls.reverse();

    let mut init_vector = vec![0; w+1];
    let mut lazy_segment_tree = lazy_segment_tree::new_range_assignment_update_and_range_maximum_query_from_vec(&init_vector);
    let mut ans = vec![h; n];

    for i in 0..n {
        // 一番下にあるやつから順に落としていく
        let (ri, ci, li, ind) = rcls[i];
        let max_height = lazy_segment_tree.range_query(ci, ci + li - 1);

        ans[ind] = h - max_height as usize;
        lazy_segment_tree.range_update(ci, ci + li - 1, max_height + 1);
