...
use superslice::*;
fn main() {
    // 2024-05-04 22:20-22:40 (20min)
    // 2024-05-05 10:32-11:36 (1h4min, 解説AC)
    // 1h24min
    input! {
        n: usize,
        m: usize,
    ...
    for edge in edges {
        ...
        let ai = edge.1;
        for j in 0..ai.len()-1 {
            if !uft.issame(ai[j] - 1, ai[j+1] - 1) {
                ans += ci;
                uft.unite(ai[j] - 1, ai[j+1] - 1);
                // println!("connected! {} - {}, ci={ci}", ai[j], ai[j+1]);
            }
        }
    }
    if uft.size(0) != n {
