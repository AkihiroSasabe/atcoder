...
fn main() {
    // 2024-03-02 15:50-?
    // 2024-03-04 20:06-20:49 (43min) 遅延評価セグメント木で解こうとして、沼る
    // 2024-03-04 20:49-22:00  (1h11min) 解説見て、回答
    // 2024-03-05 18:58-19:12 (14min)
    // Total: 2h8min = 128min

    // 解説の参考
    // https://atcoder.jp/contests/abc210/editorial/2298
    // https://note.com/momomo0214/n/na2ca58906c3f
    input! {
        h: usize,
        w: usize,
        c: isize,
    ...
    // 左上から右下に降りるパターン
    // dp[y][x] := (y,x)に至るまでに、1個の駅と線路を引いてきたときのコストの最小値
    ...
    for y in 0..h {
        for x in 0..w {
            if y == 0 && x == 0 {continue}
            if y+1 < h {
                dp[y+1][x] = min(dp[y+1][x], dp[y][x] + c);
                dp[y+1][x] = min(dp[y+1][x], a[y][x] + c);
            }
            if x+1 < w {
                dp[y][x+1] = min(dp[y][x+1], dp[y][x] + c); 
                dp[y][x+1] = min(dp[y][x+1], a[y][x] + c); 
            }
        }
    }

    // 右上から左下に降りるパターン
    // dp2[y][x] := (y,x)に至るまでに、1個の駅と線路を引いてきたときのコストの最小値
    let mut dp2 = vec![vec![INF; w]; h];
    dp2[0][w-2] = a[0][w-1] + c;
    dp2[1][w-1] = a[0][w-1] + c;
    for y in 0..h {
        for x in (0..w).rev() {
            if y == 0 && x == w-1 {continue}
            if y + 1 < h {
                dp2[y+1][x] = min(dp2[y+1][x], dp2[y][x] + c);
                dp2[y+1][x] = min(dp2[y+1][x], a[y][x] + c);
            }
            if x > 0 {
                dp2[y][x-1] = min(dp2[y][x-1], dp2[y][x] + c);
                dp2[y][x-1] = min(dp2[y][x-1], a[y][x] + c);
            }
        }
    }
    // dp.print_2d_array_with_name("dp");
    // dp2.print_2d_array_with_name("dp2");
    ...
    for y in 0..h {
        for x in (0..w).rev() {
            if y == 0 && x == w-1 {continue}
            ans = min(ans, dp2[y][x] + a[y][x]);
        }
    }
    println!("{}", ans);


    // // 全探索で、1000 x 1000 の計算時間 10^6
    // // columnの遅延評価セグメント木をW本作る
    // let mut w_trees = vec![]; // 長さw
    // let mut init_mins = vec![]; // 長さw
    ...
    //     for y in 0..h {
    //         // target駅の建設費 + 線路の建設費
    //         init_vector.push(a[y][x] + (x + y) as isize * c);
    //     }
    //     let mut lazy_segment_tree = lazy_segment_tree::new_range_increment_update_and_range_minimum_query_from_vec(&init_vector);

    //     let mut min = lazy_segment_tree.range_query(0, h-1);
    //     if x == 0 {
    //         min = lazy_segment_tree.range_query(1, h-1);
    //     }
    //     init_mins.push(min);
    //     w_trees.push(lazy_segment_tree);
    // }

    // // 範囲wの木
    // let mut global_tree = lazy_segment_tree::new_range_increment_update_and_range_minimum_query_from_vec(&init_mins);

    // // 長さwの木
    // let init_sums = vec![0; w];
    // let mut sum_tree = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_sums);
    ...
    //         if x != 0 {
    //             // 1個左のやつ
    //             w_trees[x-1].range_update(y, y, c);
    //             // 自分自身
    //             w_trees[x].range_update(y, y, -c);

    //             // 2個以上左のやつ
    //             sum_tree.range_update(0, x-2, c);
    //             // 自分より右のやつ
    //             sum_tree.range_update(x+1, w-1, -c);


    //             let v_pre = w_trees[x-1].range_query(0, h-1);
    //             let v = w_trees[x].range_query(1, h-1);

    //             ans = min(ans, v_pre + a[y][x]);
    //             ans = min(ans, v + a[y][x]);


    //             // xより左のやつは、一律で、線路代が+C円
    //             global_tree.range_update(0, x-1, c);
    //             // xとxより右のやつは、一律で、線路代が0C円
    //             global_tree.range_update(x, w-1, -c);
                

    //             // let v0 = trees[x-1].range_query(y, y);
    //             // trees[x-1].range_update(y, y, -INF);
    //             // trees[x-1].range_update(y, y, a[y][x-1] + c * (x-1) as isize);

    //             // // INF にする
    //             // let v = trees[x].range_query(y, y);
    //             // trees[x].range_update(y, y, -v + INF);


    //             let v0 = trees[x-1].range_query(0, h-1);
    //             let v1 = trees[x].range_query(1, h-1);

    //             global_tree.range_update(0, x-1, c);
    //             global_tree.range_update(x, w-1, -c);

    //             let q0 = global_tree.range_query(x-1, x-1);
    //             let q1 = global_tree.range_query(x, x);
    //             global_tree.range_update(x-1, x-1, min(v0 + c, q0));
    //             global_tree.range_update(x, x, v1);
    //         }
    //         let temp = a[y][x] + global_tree.range_query(0, w-1);
    //         ans = min(ans, temp);
    //     }
