...
fn main() {
    ...
    let mut min_d = 1_000_000_000_000_000;
    let mut max_d = 0;
    let mut temp = vec![];
    for i in 0..n {
        let di = d[i] % (a + b);
        min_d = min(min_d, di);
        max_d = max(max_d, di);
        temp.push(di as isize);
    }
    temp.sort();

    let mut min_term = 1_000_000_000_000_000;
    let modulus = (a + b) as isize;

    for i in 1..n {
        // 先頭がiのとき
        let temp_term = 1 + (temp[i - 1] + modulus - temp[i]);
        min_term = min(min_term, temp_term);
    }
    let temp_term = temp[n-1] - temp[0] + 1;
    min_term = min(min_term, temp_term);
    
    // println!("min_d = {:?}", min_d);
    // println!("max_d = {:?}", max_d);
    // if (1 + max_d - min_d) <= a {
    if min_term <= a as isize {
        println!("Yes");
    }
    else {
        println!("No");
