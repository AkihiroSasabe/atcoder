...
use rand::Rng;
fn main() {
    // 2024-08-31 21:51-22:40 (49min)
    // 2024-09-02 19:00-19:40 (40min)
    // total 89min
    input! {
        h: usize,
        w: usize,
    ...
    fn e() -> (usize, usize) {
        let init_value: (usize, usize) = (std::usize::MIN, std::usize::MIN);
        return init_value
    }
    /// 区間操作(更新)演算mappingにおける恒等写像 (lazyの初期化と、mappingとcompositionに使われる。区間代入更新のときは、正直なんでもいいと思う。)
    fn id() -> (usize, usize) {
        let init_value: (usize, usize) = (std::usize::MIN, std::usize::MIN); // 代入する値の範囲外に使われなければ、どんな値を入れてもいい。
        return init_value
    }
    ...
    for i in 0..w+1 {
        lazy_segment_tree.range_update(i, i, (0, 0));
    }

    let mut ind: usize = 0;
    let mut ind_to_yx = vec![(h-1, w-1)];
    let mut next_inds = vec![0];

    for y in (0..h).rev() {
        for i in (0..ys[y].len()).rev() {
            ind += 1;

            let (max_val, max_ind) = lazy_segment_tree.range_query(ys[y][i], w-1);
            lazy_segment_tree.range_update(ys[y][i], ys[y][i], (max_val + 1, ind));
            ind_to_yx.push((y, ys[y][i]));
            next_inds.push(max_ind);
        }
    }
    let (ans, mut ind) = lazy_segment_tree.range_query(0, w-1);
    let mut cx = 0;
    let mut cy = 0;
    println!("{}", ans);
    loop {
        let (ny, nx) = ind_to_yx[ind];
        for _ in (cx..nx) {
            print!("R");
        }
        for _ in cy..ny {
