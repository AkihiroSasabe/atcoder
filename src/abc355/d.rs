...
fn main() {
    ...
    // 座標圧縮
    let (ranked_array, _sorted_array, original_to_rank) = rank_array(&lr);

    llr.sort();
    let mut l3 = vec![];
    let mut r3 = vec![];
    let mut init_vector = vec![0; 2*n+2];
    let mut lazy_segment_tree = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    for i in 0..n {
        let li = *original_to_rank.get(&llr[i].0).unwrap();
        let ri = *original_to_rank.get(&llr[i].1).unwrap();
        l3.push(li);
        r3.push(ri);
        lazy_segment_tree.range_update(li, li, 1);
    }
    ...
    for i in 0..n {
        ...
        let num = lazy_segment_tree.range_query(li, ri).value;
        ans += num;
    }
    println!("{}", ans);
