...
use rand::Rng;
fn main() {
    // 2024-11-13 12:40-13:02 (22min)
    // 2024-11-13 20:01-21:00 (59min)
    // 2024-11-14 19:04-19:51 (47min)
    // Total: 128 min
    input! {
        n: usize,
        q: usize,
    ...
    // my_solve(n, q, h, l, r);
    educational_solve(n, q, h, l, r);


}

fn educational_solve(n: usize, q: usize, h: Vec<isize>, l: Vec<usize>, r: Vec<usize>) {
    // 2024-11-14 20:36-20:58 (22min)
    // 公式解説通りの解法.スタックの考え方の練習

    // lazy[i] = 0 -> ビルi を見ることができない
    // lazy[i] = 1 -> ビルi を見ることができる
    // lazy[L,R] -> 区間[L,R]で、見ることが可能なビルの個数
    // add, sum
    let mut init_vector = vec![0; n];
    let mut lazy: lazy_segment_tree::LazySegmentTree<lazy_segment_tree::SSum, fn(&lazy_segment_tree::SSum, &lazy_segment_tree::SSum) -> lazy_segment_tree::SSum, fn() -> lazy_segment_tree::SSum, isize, fn(&isize, &lazy_segment_tree::SSum) -> lazy_segment_tree::SSum, fn(&isize, &isize) -> isize, fn() -> isize> = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);

    let mut queries = vec![vec![]; n];
    for i in 0..q {
        queries[l[i]].push((r[i], i));
    }

    let mut ans = vec![0; q];
    let mut stack = vec![]; // leftから、見ることが可能なビルを格納
    for left in (0..n).rev() {

        // left を左端に持つクエリに、回答する。
        for (ri, i) in &queries[left] {
            if ri + 1 > n {continue}
            ans[*i] = lazy.range_query(*ri+1, n-1).value;
        }

        // 自分より、右に居て低いビルは除去 (除去する回数は、ループ全体を通してN回以下になる。)
        while stack.len() != 0 && h[stack[stack.len()-1]] < h[left] {
            let v = stack.pop().unwrap();
            lazy.range_update(v, v, -1);
        }

        // 見ることが可能なビルを追加
        stack.push(left);
        lazy.range_update(left, left, 1);
    }
    for i in 0..q {
        println!("{}", ans[i]);
    }
...
fn my_solve(n: usize, q: usize, h: Vec<isize>, l: Vec<usize>, r: Vec<usize>) {
    // 2本の遅延評価セグメント木を使う (TLEギリギリだった)

    // ビルli​とビルri​の両方から見ることができるものの個数 (公式解説通り)
    // ・ri より右にあるビル　かつ
    // ・liから見れるビル　の個数

    // btree := {クエリのr: vec![クエリの番号]}
    let mut btree = BTreeMap::new();
    let mut ans = vec![0; q];
    for i in 0..q {
        btree.entry(r[i]).or_insert(vec![]).push(i);
    }

    // lazy[L,R] := 区間[L,R] の中で最大のビルの高さ
    // rep, max
    let init_vector = h.clone();
    let mut lazy_max: lazy_segment_tree::LazySegmentTree<isize, fn(&isize, &isize) -> isize, fn() -> isize, isize, fn(&isize, &isize) -> isize, fn(&isize, &isize) -> isize, fn() -> isize> = lazy_segment_tree::new_range_assignment_update_and_range_maximum_query_from_vec(&init_vector);

    // lazy[i] := 場所i から見えるビルの数
    // add, sum
    let mut init_vector = vec![0; n];
    let mut lazy: lazy_segment_tree::LazySegmentTree<lazy_segment_tree::SSum, fn(&lazy_segment_tree::SSum, &lazy_segment_tree::SSum) -> lazy_segment_tree::SSum, fn() -> lazy_segment_tree::SSum, isize, fn(&isize, &lazy_segment_tree::SSum) -> lazy_segment_tree::SSum, fn(&isize, &isize) -> isize, fn() -> isize> = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    ...
    // 計算量: O(N logN * logN)
    // 後ろから考える。
    let mut ans: Vec<isize> = vec![0; q];
    for ri in (1..n).rev() {
        // println!("---- i = {:?}", i);

        // ri を区間の右端点に持つクエリが居たら、そのクエリの答えが確定する。
        if let Some(queries_for_ri) = btree.get(&ri) {
            for &q_ri_index in queries_for_ri {
                let li = l[q_ri_index];
                let num: isize = lazy.range_query(li, li).value;
                // println!("li = {}, num = {}", li, num);
                ans[q_ri_index] = num;
            }
        }
        
        // left を二分探索で求める。
        // left := h[ri] より大きくて、一番左の場所
        let mut ng = ri; // ng -ri: H[ri]より高い奴がいない
        let mut ok = 0; // ok - ri: H[ri]より高い奴がいる
        let mut left = 0;
        if !judge(ok, h[ri], ri, &mut lazy_max) {
            // 前方に h[i] より高いビルがいない場合
            left = 0;
        }
        else {
            // 二分探索
            while (ng as i128 - ok as i128).abs() > 1 {
                let mid = (ng + ok) / 2;
                let is_ok = judge(mid, h[ri], ri, &mut lazy_max);
                if is_ok {
                    ok = mid;
                }
                else {
        ...
        // [left, ri-1]は、 ri を見ることが出来るので、+1 する。
        lazy.range_update(left, ri-1, 1);
    }
...
// めぐる式二分探索
fn judge(mid: usize, hi: isize, right: usize, lazy_max: &mut lazy_segment_tree::LazySegmentTree<isize, fn(&isize, &isize) -> isize, fn() -> isize, isize, fn(&isize, &isize) -> isize, fn(&isize, &isize) -> isize, fn() -> isize>) -> bool {
    let hmax = lazy_max.range_query(mid, right);
    return hmax > hi
}
