...
fn main() {
    // 2025-03-04 20:42-20:59 (17min, ひらめき)
    // 2025-03-04 21:00-21:16 (16min, 実装)
    // 33min
    input! {
        n: usize,
        q: usize,
    ...
    for i in 0..q {
        ...
        while a2.len() > 0 && xi >= a2[a2.len()-1].0 {
            // xi より小さいやつを全部、セグ木に記録していく
            let (ai, ind2) = a2.pop().unwrap();
            let ind2 = (- ind2) as usize;
            let val = lazy_segment_tree.range_query(0, ind2);
            lazy_segment_tree.range_update(ind2, ind2, val+1);
        }
        ans[ind] = lazy_segment_tree.range_query(0, ri);
    }
    for i in 0..q {
        println!("{} ", ans[i]);
    }
