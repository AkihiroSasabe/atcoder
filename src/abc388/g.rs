...
fn main() {
    ...
    for i in 0..n {
        let min_opposite = a.lower_bound(&(a[i] * 2));
        min_lengths.push((min_opposite - i) as isize)
    }

    // let init_vector = min_lengths;
    // let mut lazy: lazy_segment_tree::LazySegmentTree<isize, fn(&isize, &isize) -> isize, fn() -> isize, isize, fn(&isize, &isize) -> isize, fn(&isize, &isize) -> isize, fn() -> isize> = lazy_segment_tree::new_range_assignment_update_and_range_maximum_query_from_vec(&init_vector);

    let mut lazy = SegmentTree::new(n);
    for i in 0..n {
        lazy.update(i, min_lengths[i]);
    }

    fn judge(mid: usize, i: usize, ls: &Vec<usize>, rs: &Vec<usize>, 
        lazy: &mut SegmentTree
        // lazy: &mut lazy_segment_tree::LazySegmentTree<isize, fn(&isize, &isize) -> isize, fn() -> isize, isize, fn(&isize, &isize) -> isize, fn(&isize, &isize) -> isize, fn() -> isize>
    ) -> bool {
        // let mut length = lazy.range_query(ls[i], ls[i] + mid as usize - 1);
        let mut length = lazy.range_max_query(ls[i], ls[i] + mid as usize - 1);
        length = max(length, mid as isize);
        let last_index = (ls[i] + mid) as isize - 1 + length;
        // println!("length = {:?}", length);
        // println!("last_index = {:?}", last_index);
        return last_index <= rs[i] as isize
    }
    for i in 0..q {
        // めぐる式二分探索
        // 関数じゃなくて、クロージャーを使うと、引数を少なく出来る。
        // println!("ls[{i}] = {}, rs[{i}] = {:?} --------------------------", ls[i], rs[i]);
        ...
        while (ng as i128 - ok as i128).abs() > 1 {
            ...
            // println!("mid = {:?}", mid);
            // let is_ok = judge(mid);
            let is_ok = judge(mid, i, &ls, &rs, &mut lazy);
            if is_ok {
                ok = mid;
            }
            else {
...
//         let ri = rs[i];
//         let index = li / width;
//         // section[index].push(vec![ri, li, i]);
...
//             let ri = section[i][j].0;
//             let li = section[i][j].1;
//             let ind = section[i][j].2;
//             println!("ri = {}, li = {}, ind = {}", ri, li, ind);
//             if j == 0 {
//                 temp = solve(&a[li..ri+1]);
//                 ans[ind] = temp;
//                 println!("temp = {:?}", temp);
//                 continue
//             }
//             let mut pre_ri = section[i][j-1].0;
//             let mut pre_li = section[i][j-1].1;
//             let pre_ind = section[i][j-1].2;
//             let mut pre_ans = ans[pre_ind];

//             while pre_li != li {
//                 // 左側が広がるケース
//                 if li < pre_li {
//                     let ll = pre_li-1;
//                     if a[ll] * 2 <= a[pre_ri-pre_ans] {
//                         pre_ans += 1;
//                     }
//                     pre_li = ll;
//                 }
