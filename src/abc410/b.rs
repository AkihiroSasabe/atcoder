...
fn main() {
    ...
    // solve_real_time(n,q,x);
    solve_after_contest(n,q,x);

}
fn solve_after_contest(n: usize, q: usize, x: Vec<usize>) {
    // 2025-06-15 9:36-9:40 (4min)
    let mut balls = vec![0; q];
    let mut boxes: Vec<usize> = vec![0; n+1];
    boxes[0] = 1 << 60;
    ...
    for i in 0..q {
        if x[i] > 0 {
            ...
            let min_num = *boxes.iter().min().unwrap();
            for b in 1..n+1 {
                if boxes[b] == min_num {
                    balls[i] = b;
                    boxes[b] += 1;
    ...
    // for i in 0..q {
    println!("{}", balls.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(" "));
    // }

}   

fn solve_real_time(n: usize, q: usize, x: Vec<usize>) {
    ...
    // let mut lazy = lazy_segment_tree::new_range_assignment_update_and_range_minimum_query_from_vec(&init_vector);

    let init_vector = vec![0; n];
    let mut lazy = lazy_segment_tree::new_range_increment_update_and_range_minimum_query_from_vec(&init_vector);

    let mut boxes = vec![0; n];
    let mut xx = vec![0; q];
    for i in 0..q {
        let min_num = lazy.range_query(0, n-1) as usize;
        if x[i] == 0 {
            for j in 0..n {
                if boxes[j] == min_num {
                    boxes[j] += 1;
                    xx[i] = j + 1;
                    lazy.range_update(j, j, 1);
                    break;
                }
            }
        }
            ...
            boxes[x[i] - 1] += 1;
            xx[i] = x[i];
            lazy.range_update(x[i]-1, x[i]-1, 1);
        }
    }

    println!("{}", xx.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(" "));
