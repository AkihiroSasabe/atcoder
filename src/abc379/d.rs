...
fn solve_deque(q: usize, k: Vec<usize>, t: Vec<isize>) {
    // 2024-11-12 20:09-20:46 (37min, 解き直し)
    // クエリ1：鉢を1個用意。クエリ2: T日待つ。クエリ3: Hi以上の木を栽培し、本数を答える問題。
    ...
    for i in 0..q {
        match k[i] {
            1 => {
                // 今のcumより後ろの奴から足す
                queue.push_back(cum.len());
            },
            2 => {
                cum.push(t[i]);
                let len = cum.len()-1;
                cum[len] = cum[len] + cum[len-1];
            }
            3 => {
                if queue.len() != 0 {
                    let mut ans = 0;
                    while let Some(index) = queue.pop_front() {
                        // 刈れるか?
                        let height = cum[cum.len()-1] - cum[index-1];
                        if height >= t[i] {
                            ans += 1;
                        }
                        else {
...
fn my_solve(q: usize, k: Vec<usize>, t: Vec<isize>) {
    let mut sum_ans = 0; // 刈り取った植木鉢の合計
    let mut num = 0; // 植木鉢の数

    // increase, max
    let init_vector = vec![0; q+2];
    let mut lazy_segment_tree: lazy_segment_tree::LazySegmentTree<isize, fn(&isize, &isize) -> isize, fn() -> isize, isize, fn(&isize, &isize) -> isize, fn(&isize, &isize) -> isize, fn() -> isize> = lazy_segment_tree::new_range_increment_update_and_range_maximum_query_from_vec(&init_vector);
    ...
    for i in 0..q {
        if k[i] == 1 {      
            ...
            let ti = t[i];
            lazy_segment_tree.range_update(0, num, ti);
        }
        else if k[i] == 3 {
            // かる
            // にぶたん
            // めぐる式二分探索
            // 刈り取れる植木鉢の最大のindexが知りたい
            let hi = t[i];
            let max_h = lazy_segment_tree.range_query(sum_ans+1, q+1);
            if max_h < hi {
                println!("0");
                continue
            }

            // ok := 刈り取れる植木鉢の最大のindex
            let mut ng = q+1;
            let mut ok = sum_ans+1;

            // 
            fn judge(mid: usize, q: usize, hi: isize, lazy: &mut lazy_segment_tree::LazySegmentTree<isize, fn(&isize, &isize) -> isize, fn() -> isize, isize, fn(&isize, &isize) -> isize, fn(&isize, &isize) -> isize, fn() -> isize>) -> bool {
                let height = lazy.range_query(mid, q+1);
                return height >= hi 
            }

            while (ng as i128 - ok as i128).abs() > 1 {
                let mid = (ng + ok) / 2;
                let is_ok = judge(mid, q, hi, &mut lazy_segment_tree);
                if is_ok {
                    ok = mid;
                }
                else {
