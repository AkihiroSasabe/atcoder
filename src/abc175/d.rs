...
use superslice::*;
fn main() {
    // 2024-04-04 20:32-20:58 (26min)
    // 2024-04-05 21:01-22:00 (59min)
    // 2024-04-06 10:30-10:59 (29min)
    // total 114min
    input! {
        n: usize,
        k: usize,
    ...
    let mut ans = - 1 << 60;
    for cyc in cycs.iter() {

        let mut seg = lazy_segment_tree::new_range_increment_update_and_range_maximum_query_from_vec(cyc);
        ...
        // println!("num = {:?}", num);
        // 1周がプラスか?
        let sum = seg.range_query(num-1, num-1);
        // println!("sum = {:?}", sum);
        for i in 0..num {
            // println!("i = {i}*****************");
            // 先頭がiのとき
            if i != 0 {
                // セグ木の更新
                let pre = seg.range_query(i-1, i-1);
                let now = seg.range_query(i, i);
            ...
            if sum > 0 {
                // 全周する
                let mut temp = term as isize * sum;
                ans = max(ans, temp);
                if r > 0 {
                    let mut temp2 = seg.range_query(i, min(i + r-1, num-1));
                    if i + r > num {
                        let temp3 = seg.range_query(0, i+r-1 - num);
                ...
                // 全周 - 1する
                if term > 0 {
                    let mut temp = (term - 1) as isize * sum;
                    ans = max(ans, temp);
                    let temp2 = seg.range_query(0, num-1);
                    // println!("temp2 = {:?}", temp2);
                    temp += temp2;
                    ans = max(ans, temp);
                    // println!("temp b= {:?}", temp);
                ...
                let mut temp = seg.range_query(i, min(num-1, i + k-1));
                if i + k - 1 > num - 1 {
                    let temp2 = seg.range_query(0, i+k-1-num);
                    temp = max(temp, temp2);
                }
                // println!("temp c= {:?}", temp);
                ans = max(ans, temp);
