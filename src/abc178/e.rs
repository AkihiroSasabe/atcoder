...
fn main() {
    // 2025-05-24 10:36-12:02 (1h26min)
    // 2025-05-24 15:30-16:21 (51min)
    // Total: 2h17min
    
    // たぶん、遅延評価セグメント木で解けない問題!!
    // 区間加算更新 & 区間最大取得において、最大取得演算に"絶対値"がでてくると、
    // 遅延評価ができなくなる。 (遅延更新はできるけど、遅延取得ができない)
    // マンハッタン距離は、45度回して解くのが鉄則!
    assert_brute_force();
}

// abc178-e
fn assert_brute_force() {
    // 愚直な解と不整合が出るようなテストケースを発見する関数。
    // 不整合が出るまで、ランダムなテストケースを生成し続ける。
...
*/
    ...
    loop {
        let (n, x, y) = random_input();
        let ans_bf = solve_bf(n,x.clone(),y.clone());
        let ans = solve(n,x,y);
        println!("ans_bf = {}", ans_bf);
        println!("ans = {}", ans);
        println!("------");
        if ans != ans_bf {
...
fn random_input() -> (usize, Vec<isize>, Vec<isize>) {
    // 乱数を使って、適当なテストケースを作成する
    // n: usize
    // x: Vec<isize>
    // y: Vec<isize>
    use rand::Rng;
    ...
    let x = (0..n).map(|_| rng.gen_range(0..=4)).collect::<Vec<_>>();
    let y = (0..n).map(|_| rng.gen_range(0..=4)).collect::<Vec<_>>();
    println!("{}",n);
    for i in 0..n {
        println!("{} {}", x[i], y[i]);
    }
...
fn input() -> (usize, Vec<isize>, Vec<isize>) {
    input! {
        n: usize,   
    }
    let mut x = vec![];
...
fn solve_bf(n: usize, x: Vec<isize>, y: Vec<isize>) -> isize {
    let mut ans = 0;
    for i in 0..n {
        for j in i..n {
            let cand = (x[i] - x[j]).abs() + (y[i] - y[j]).abs();
            ans = max(ans, cand);
        }
    }
    return ans
...
fn solve(n: usize, x: Vec<isize>, y: Vec<isize>) -> isize {
    // マンハッタン距離 (:= |x0-x1| + |y0-y1|) の問題は、
    // 座標系(x,y)を45度左回りに回転させた座標系(X,Y)を考える。
    // そうすると、p0とp1のマンハッタン距離は、max(|X0-X1|, |Y0-Y1|)となるため、1次元の問題に落とし込みやすい。
    // 45度回転させると、(x,y) -> (X,Y) = (x+y, -x+y) となる。
    // (X,Y) = M @ [x,y] となる変換行列Mの求め方は、以下の通り。
    // 例えば、(x,y)=(1,0)が(X,Y)=(1,-1)に、(x,y)=(0,1)が(X,Y)=(1,1)になるので、そんな変換行列Mを考えれば,
    // [[1, 1],
    // [-1,1]]
    // となることにすぐ気がつく。注意がいるのは、この変換で、√2倍の拡大が起きること。
    let mut X = vec![];
    let mut Y = vec![];
    for i in 0..n {
        let xi = x[i];
    ...
    X.sort();
    Y.sort();
    let ans = max(X[X.len()-1] - X[0], Y[Y.len()-1] - Y[0]);
    return ans;

    // 以下、遅延評価セグメント木を使った誤った解法
    fn op(&s0: &(isize, isize), &s1:  &(isize, isize)) -> (isize, isize) {
        // s0.max(s1)
        let s00 = max(s0.0.abs(), s0.1.abs());
        let s11 = max(s1.0.abs(), s1.1.abs());
        if s00 >= s11 {
            return s0
        }
        else {
    ...
    fn mapping(&f: &(isize, isize) ,&x: &(isize, isize)) -> (isize, isize) {
        (f.0 + x.0, f.1 + x.1)
    }
    fn composition(&f: &(isize, isize) ,&g: &(isize, isize)) -> (isize, isize) {
        // f + g
        (f.0 + g.0, f.1 + g.1)
    }
    fn e() -> (isize, isize) {
        ...
        // let init_value: isize = - 1_000_000_000_000_000_00;
        return (init_value, init_value)
    }
    fn id() -> (isize, isize) {
    ...
    for i in 0..n {
        ...
        let Yi = -xi + yi;
        // println!("Xi, Yi = {:?}", (Xi, Yi));
        lazy_segment_tree.range_update(i, i, (Xi, Yi));
    }
    lazy_segment_tree.print_all_single_queries();
    ...
    for i in 0..n {
        ...
        let Xi = xi + yi;
        let Yi = -xi + yi;
        lazy_segment_tree.range_update(0, n-1, (-Xi, -Yi));
        lazy_segment_tree.print_all_single_queries();        

        let val = lazy_segment_tree.range_query(0, n-1);
        let cand: isize = max(val.0.abs(), val.1.abs());
        println!("i = {i}, val = {:?}, cand = {:?}", val, cand);
        ans = max(ans, cand);
        lazy_segment_tree.range_update(0, n-1, (Xi, Yi));
    }
    // println!("{}", ans);
