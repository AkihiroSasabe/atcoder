...
use rand::Rng;
fn main() {
    // 2024-08-26 20:31-21:00 (29min)
    // 2024-08-27 12:46-13:00 (14min)
    // 2024-08-27 19:12-20:33 (1h21min)
    // Total: 2h4min
    input! {
        n: usize,
        mut a: [usize; n],
    ...
    let mut init_vector = vec![];
    for i in 0..n {
        init_vector.push(a[i] as isize);
    }
    let mut tree_a = lazy_segment_tree::new_range_assignment_update_and_range_sum_query_from_vec(&init_vector);

    let mut over_1_pos = BTreeSet::new();
    for i in 0..n {
        if b[i] > 1 {
            over_1_pos.insert(i);
        }
    ...
    // let mut init_vector = vec![];
    // for i in 0..n {
    //     init_vector.push((b[i], i));
    // }
    // // new_range_assignment_update_and_range_maximum_query_from_vec を書き換えた。
    // let mut tree_b = lazy_segment_tree::new_range_assignment_update_and_range_maximum_query_from_vec(&init_vector);
    // tree_b.print_all_single_queries();

    for i in 0..q {
        if t[i] == 1 {
            // a[l[i]] を、 r[i]+1 に置き換える
            let ind = l[i];
            let x = r[i] + 1;
            a[ind] = x;
            tree_a.range_update(ind, ind, x as isize);
        }
        else if t[i] == 2 {
            // b[l[i]] を、 r[i]+1 に置き換える
            let ind = l[i];
            let x = r[i] + 1;
            b[ind] = x;

            if x <= 1 {
                over_1_pos.remove(&ind);
            }
            else {
                over_1_pos.insert(ind);
            }
            // tree_b.range_update(ind, ind, (x, ind));
        }
        else if t[i] == 3 {
            // j = l[i], ..., r[i] の順に、
            // v = v + a[j] または
            ...
            // v < 10^18より、
            // B[i] が2を超えているのは、60個しかない。
            // それ以外は1なので、a[i]の和を取っておけば良い。
            // 各クエリで、O(60) しかかからない。
            // 問題は、どこに2以上があるか
            // rep, range_max のセグ木 を 60回やるだけでいけそう。
            ...
            // loop {
            //     let (val, ind) = tree_b.range_query(li+1, ri);
            //     if val <= 1 {break}
            //     set.insert((ind, val));
            //     tree_b.range_update(ind, ind, (0, ind));
            // }

            let mut now_ind = li + 1;
            // println!("-------------");
            // println!("a = {:?}", a);
            // println!("b = {:?}", b);
            // println!("set = {:?}", set);
            ...
            for &ind in over_1_pos.iter() {
                if ind > ri {break;}
                // println!("now_ind = {now_ind}, ind = {:?}, v = {v}",ind);
                if now_ind == ind {
                    v = max(v+a[now_ind], v * b[ind]);
                    now_ind = ind + 1;
                }
                else if now_ind < ind {
                    let cum_a =  tree_a.range_query(now_ind, ind-1).value;
                    v = v + cum_a as usize;
                    now_ind = ind;
                    v = max(v+a[now_ind], v * b[ind]);
                    now_ind = ind + 1;
                ...
                // println!("v = {:?}", v);
                // tree_b.range_update(ind, ind, (val, ind));
            }
            // tree_b.print_all_single_queries();
            if now_ind <= ri {
                let cum_a =  tree_a.range_query(now_ind, ri).value;
                v = v + cum_a as usize;
            }
            println!("{}", v);
        }
...
pub mod lazy_segment_tree {
    ...
    pub fn new_range_assignment_update_and_range_maximum_query(list_size: usize) -> 
    LazySegmentTree<(usize, usize), fn(&(usize, usize), &(usize, usize)) -> (usize, usize), fn() -> (usize, usize), (usize, usize), fn(&(usize, usize), &(usize, usize)) -> (usize, usize), fn(&(usize, usize), &(usize, usize)) -> (usize, usize), fn() -> (usize, usize)> {
        /// 区間取得演算 (opだけど、更新操作ではなく取得なので注意)
        /// s0: 左の子data (たぶん)
        /// s1: 右の子data (たぶん)
        /// 返り値: 2子から得られたdata
        fn op(&s0: &(usize, usize), &s1:  &(usize, usize)) -> (usize, usize) {
            s0.max(s1)
        }
        /// Lazy -> Data の更新操作
        /// f: Lazyによる操作
        /// x: 更新前のdata
        /// 返り値: 更新後のdata
        fn mapping(&f: &(usize, usize) ,&x: &(usize, usize)) -> (usize, usize) {
            if f == id() {
                return x
            }
            else {
        ...
        /// g: これまでの更新操作
        /// 返り値: g -> f の順にする操作。gとfの合成関数: f(g(x))
        fn composition(&f: &(usize, usize) ,&g: &(usize, usize)) -> (usize, usize) {
            if f == id() {
                return g
            }
            else {
        ...
        /// 区間取得演算opの単位元 (dataの初期化に使われる)
        fn e() -> (usize, usize) {
            let init_value: usize = std::usize::MIN;
            return (init_value, 0)
        }
        /// 区間操作(更新)演算mappingにおける恒等写像 (lazyの初期化と、mappingとcompositionに使われる。区間代入更新のときは、正直なんでもいいと思う。)
        fn id() -> (usize, usize) {
            let init_value: usize = std::usize::MIN; // 代入する値の範囲外に使われなければ、どんな値を入れてもいい。
            return (init_value, 0)
        }
        let lazy_segment_tree = LazySegmentTree::new(
            list_size,
            e as fn() -> (usize, usize), 
            op as fn(&(usize, usize), &(usize, usize)) -> (usize, usize), 
            mapping as fn(&(usize, usize), &(usize, usize)) -> (usize, usize), 
            composition as fn(&(usize, usize), &(usize, usize)) -> (usize, usize), 
            id as fn() -> (usize, usize)
        );
        return lazy_segment_tree
    }

    // [5] 区間代入更新・区間最大値取得 を vector から生成
    pub fn new_range_assignment_update_and_range_maximum_query_from_vec(init_vector: &Vec<(usize, usize)>) -> LazySegmentTree<(usize, usize), fn(&(usize, usize), &(usize, usize)) -> (usize, usize), fn() -> (usize, usize), (usize, usize), fn(&(usize, usize), &(usize, usize)) -> (usize, usize), fn(&(usize, usize), &(usize, usize)) -> (usize, usize), fn() -> (usize, usize)> {
        let n = init_vector.len();
