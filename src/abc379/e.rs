...
use rand::Rng;
fn main() {
    // 2024-11-11 12:19-12:48 (29min)
    // 2024-11-11 20:20-20:50 (30min)
    // 59min
    input! {
        n: usize,
        s: Chars
    }
    // s[i] の寄与: s[i] * (1 + 10 + ...10^(N-1-i)) * (i + 1) となることが、実験からわかる。
    // あとは、寄与をまとめて考えれば、O(N) で求まる。 そのまま計算すると、オーバーフローするので、桁毎に、リストを持たせる。
    // (例) 379の場合
    // [1,1]: 3
    // [1,2]: 37
    // [1,3]: 379
    ...
    // s[0] の寄与: s[0] * (1 + 10 + 100) * 1
    // s[1] の寄与: s[1] * (1 + 10) * 2
    // s[2] の寄与: s[2] *  1 * 3
    // ...
    // s[i] の寄与: s[i] * (1 + 10 + ...10^(N-1-i)) * (i + 1)

    let mut ss = vec![];
    for i in 0..n {
        ss.push(s[i] as isize - '0' as isize);
    }
    // println!("ss = {:?}", ss);
    ...
    let mut dub = 1;
    for i in 0..n {
        lazy_segment_tree.range_update(0, n-1-i, dub * ss[i]);
        // ans = ans + dub * ss[i] * cum10[n-1-i];
        dub += 1;
    }

    let mut fin = vec![];
    for i in 0..n {
        let val = lazy_segment_tree.range_query(i, i).value;
        fin.push(val);
    }
    // println!("fin = {:?}", fin);
    let mut ind = 0;
    loop {
        if fin.len() == ind {break}
        let mut ind2 = ind;
        let mut value = fin[ind];
        fin[ind] = 0;
        while value != 0 {
            let r = value % 10;
            value /= 10;
