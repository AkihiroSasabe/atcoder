...
fn main() {
    ...
    for i in 0..m {
        ...
        let len_range = (ri - li + 1) as isize;
        // println!("li: {}, ri: {}", li, ri);
        let sum = lazy_segment_tree.range_query(li, ri).value % modulus;
        let expect = mod_dev(sum as usize, len_range as usize, modulus as usize) as isize;
        lazy_segment_tree.range_update(li, ri, expect);
    }
    let mut ans = vec![];
    for i in 0..n {
        ans.push(lazy_segment_tree.range_query(i, i).value % modulus);
    }
    for i in 0..n {
        print!("{} ", ans[i]);
    }
