...
fn main() {
    ...
    let mut lazy_add_sum = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    
    let mut init_vector = vec![];
    for i in 0..n {
        init_vector.push(h[i] as isize);
    }
    ...
    for i in 1..n {
        // めぐる式二分探索
        let mut ng = i-1;
        let mut ok = 0; // 自分より高い奴
        if !judge(ok, &h, i, &mut lazy_max) {
            //自分より高い奴が前方に 存在しない
            lazy_add_sum.range_update(0, i-1, 1);
            continue
        }

        if judge(ng, &h,i, &mut lazy_max) {
            ok = ng;
        }
        while (ng as i128 - ok as i128).abs() > 1 {
            let mid = (ng + ok) / 2;
            let is_ok = judge(mid, &h, i, &mut lazy_max);
            if is_ok {
                ok = mid;
            }
            else {
        ...
        // 自分より、高いやつで最も手前が、ok
        lazy_add_sum.range_update(ok, i-1, 1);
    }

    for i in 0..n {
        let val = lazy_add_sum.range_query(i, i).value;
        print!("{} ", val);
    }
}

// 最も近くて、自分よりデカいやつ
fn judge(mid: usize, h: &Vec<usize>, my_ind: usize, lazy_max: &mut lazy_segment_tree::LazySegmentTree<isize, fn(&isize, &isize) -> isize, fn() -> isize, isize, fn(&isize, &isize) -> isize, fn(&isize, &isize) -> isize, fn() -> isize>) -> bool {
    let val = lazy_max.range_query(mid, my_ind-1);
    let my_h = h[my_ind];
    return val as usize > my_h
}
