...
fn main() {
    ...
    let mut init_vector = a.clone();
    let mut lazy = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);


    for i in 0..n-1 {
        let mut max_num_give = (n-1-i) as isize;
        let num_i = lazy.range_query(i, i).value;

        let mut num_give = 0;
        if max_num_give <= num_i {
            num_give = max_num_give;
        }
        else {
            num_give = num_i;
        }
        lazy.range_update(i, i, -num_give);
        lazy.range_update(i+1, i+num_give as usize, 1);
    }
    for i in 0..n {
        let num_i = lazy.range_query(i, i).value;
        print!("{} ", num_i);
