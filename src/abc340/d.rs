...
fn main() {
    ...
    for i in 0..n-1 {
        ...
        b.push(bi);
        x.push(xi);
        graph[i].push(Edge {neighbor: i + 1, weight: ai});

        graph[i].push(Edge {neighbor: xi, weight: bi});
    }
    let distance_list_from_0 = get_minimum_distance(&graph, 0);
    println!("{}", distance_list_from_0[n-1]);
