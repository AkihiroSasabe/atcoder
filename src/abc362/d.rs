...
fn main() {
    ...
    for i in 0..m {
        ...
        // graph[ui-1].push((vi-1, bi));
        // graph[vi-1].push((ui-1, bi));
        graph[ui - 1].push(Edge {neighbor: vi - 1, weight: bi});
        graph[vi - 1].push(Edge {neighbor: ui - 1, weight: bi});
    }

    let distance_list_from_1 = get_minimum_distance(&graph, 0, a);

    for i in 1..n {
        print!("{} ", distance_list_from_1[i]);
    }
...
mod dijkstra_algorithm {
    ...
    pub fn get_minimum_distance(graph: &Vec<Vec<Edge>>, start_v: usize, a: Vec<usize>) -> Vec<usize> {
        // ヒープを使ったダイクストラ法
        ...
        while !heap.is_empty() {
            ...
            for edge in graph[min_v].iter() {
                // 緩和できる場合
                if distance[edge.neighbor] > distance[min_v] + edge.weight + a[edge.neighbor] {
                    // 緩和
                    distance[edge.neighbor] = distance[min_v] + edge.weight + a[edge.neighbor];
...
// fn bfs(v0: usize, graph: &Vec<Vec<(usize, usize)>>) -> Vec<usize> {
//     // v0が始点
