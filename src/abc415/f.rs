...
fn main() {
    // 2025-07-20 12:34-13:04 (30min)
    // 2025-07-20 14:51-16:26 (35min)
    // 65min
    input! {
        n: usize,
        q: usize,
    ...
    let mut s = vec![];
    for i in 0..n {
        s.push((ss[i] as usize - 'a' as usize) as usize);
    }
    ...
    for i in 0..q {
        ...
        if ti == 1 {
            ...
            x.push((xi as usize - 'a' as usize) as usize);
        }
        else if ti == 2 {
            input!{
                xi: usize,
    ...
    #[derive(PartialEq)]
    pub struct SSS {
        pub length: usize, // 区間内で最大の連続文字列の長さ
        pub left: usize, // 区間内で最大の連続文字列の左端
        pub right: usize, // 区間内で最大の連続文字列の右端
    }

    fn op(&s0: &SSS, &s1:  &SSS) -> SSS {
        let aaa0: (usize, usize, usize) = (s0.length, s0.left, s0.right);
        let aaa1 = (s1.length, s1.left, s1.right);
        if aaa0 >= aaa1 {
            return s0;
        }
        else {
    ...
    /// x: 更新前のdata
    /// 返り値: 更新後のdata
    fn mapping(&f: &SSS ,&x: &SSS) -> SSS {
        let ff = (f.length, f.left, f.right);
        let idd = (id().length, id().left, id().right);
        if ff == idd {
            return x
        }
        else {
    ...
    fn composition(&f: &SSS ,&g: &SSS) -> SSS {
        let ff = (f.length, f.left, f.right);
        let idd = (id().length, id().left, id().right);
        if ff == idd {
            return g
    ...
    fn e() -> SSS {
        let init_value = SSS{length: 0, left: 0, right: 0};
    ...
    let mut lazy = lazy_segment_tree::LazySegmentTree::new(
        n,
        e,
        op,
        mapping,
    ...
    let mut inits = vec![SSS{length: 1, left: 0, right: 0}; n];
    for i in 1..n {
        inits[i].left = i;
        inits[i].right = i;
    }
    for i in 1..n {
        if s[i] == s[i-1] {
            inits[i].left = inits[i-1].left;
            inits[i].length = inits[i-1].length + 1;
        }
    }
    for i in (0..n-1).rev() {
        if s[i] == s[i+1] {
            inits[i].right = inits[i+1].right;
            inits[i].length = inits[i+1].length;
        }
    }
    // debug
    // for i in 0..n {
    ...
    for i in 0..q {
        // // debug
        // println!("(i, s) = {:?}",(i, s.clone()));
        // lazy.print_all_single_queries();
        ...
        if ti == 1 {
            ...
            if s[li] == xi {continue;}
            s[li] = xi;
            if li != 0 && li != n-1 && s[li-1] == xi && s[li+1] == xi {
                // 両隣とくっついた場合
                let left = lazy.range_query(li-1, li-1).left;
                let right = lazy.range_query(li+1, li+1).right;
                let len = right - left + 1;
                lazy.range_update(left, right, SSS{length: len, left: left, right: right});
            }
            else if li != 0 && s[li-1] == xi {
                // 左隣とだけくっついた場合
                let left = lazy.range_query(li-1, li-1).left;
                let len = li - left + 1;
                // 左側の更新
                lazy.range_update(left, li, SSS{length: len, left: left, right: li});

                // 右側の更新
                if li != n-1 {
                    let right = lazy.range_query(li+1, li+1).right;
                    lazy.range_update(li+1, right, SSS{length: right - li, left: li+1, right: right});
                }
            }
            else if li != n-1 && s[li+1] == xi {
                // 右隣とだけくっついた場合
                let right = lazy.range_query(li+1, li+1).right;
                // 右側の更新
                lazy.range_update(li, right, 
                    SSS{length: right - li + 1, left: li, right: right});
                // 左側の更新
                if li != 0 {
                    let left = lazy.range_query(li-1, li-1).left;
                    lazy.range_update(left, li-1, 
                        SSS{length: li - left, left: left, right: li-1}
                    );
                }
            }
            else {
                // liの更新
                lazy.range_update(li, li, SSS{length: 1, left: li, right: li});
                // 左側の更新
                if li != 0 {
                    // 孤立する場合
                    let left = lazy.range_query(li-1, li-1).left;
                    lazy.range_update(left, li-1, SSS{length: li - left, left: left, right: li-1});
                }
                // 右側の更新
                if li != n-1 {
                    let right = lazy.range_query(li+1, li+1).right;
            ...
            let ri = xi - 1;
            // println!("(li, ri) = {:?}", (li, ri));
            let SSS { length: leng, left: lef, right: rig} = lazy.range_query(li, ri);
            let mut ans = 0;
            if li <= lef && rig <= ri {
                // 対象の文字列が全部内側に収まっている場合
                ans = leng;
            }
            else if lef <= li && ri <= rig {
                // 対象の文字列の両端がはみ出ている場合
                ans = ri - li + 1;
            }
            else if lef < li && rig <= ri {
                // 左だけはみ出ている場合
                ans = rig - li + 1;

                // 右側もチェック
                if rig != ri {
                    let SSS { length: leng2, left: lef2, right: rig2} = lazy.range_query(rig+1, ri);
                    if rig2 <= ri {
                        ans = max(ans, leng2);
                    }
                    else {
                        ans = max(ans, ri - lef2 + 1);
                        let SSS { length: leng3, left: lef3, right: rig3} = lazy.range_query(rig+1, lef2-1);
                        ans  = max(ans, leng3);
                    }
                }
            }
                ...
                // 左側もチェック
                if lef != li {
                    let SSS { length: leng2, left: lef2, right: rig2} = lazy.range_query(li, lef-1);
                    if lef2 >= li {
                        // 左側がはみ出ていない場合
                        ans = max(ans, leng2);
                    }
                        ...
                        // 左側がはみ出ている場合
                        ans = max(ans, rig2 - li + 1);
                        let SSS { length: leng3, left: lef3, right: rig3} = lazy.range_query(rig2+1, lef-1);
                        ans  = max(ans, leng3);
                    }
                }
            }
