...
use rand::Rng;
fn main() {
    // 2024-11-24 15:59-17:00 (1h1min)
    // 2024-11-27 19:20-20:14 (54min)
    // Total: 1h55min
    input! {
        n: usize,
        k: usize,
    ...
    let inv_num_inner = get_inv_num(&inners);

    // num_range := シャッフル可能な開始位置の個数 (= n - k + 1)
    let mut num_range = 0;
    for i in 0..n {
        if i + k - 1 > n - 1 {continue}
        num_range += 1;
    }
    num_range = num_range % modulus;
    ...
    // 期待値の分子（一部の寄与）・分母（全部）を求めておく。
    let mut top = 0;

    // シャッフルする内部の転倒数の寄与を先に合算しておく
    let mut sum = 0; // とあるシャッフル区間について、全シャッフルのパターンで転倒数の和を取ると、毎回固定値になる。
    for i in 0..k {
        sum += i;
        sum %= modulus;
    }
    let init = mod_dev(num_range * factorial % modulus * sum % modulus, 2, modulus);

    top += init;
    top %= modulus;
    let mut bottom = num_range * factorial % modulus; // 事象の数 = シャッフル開始位置の範囲 * K!

    // 初期状態 のシャッフル外側の集合
    let mut temp_outer = vec![];
    let mut init_vector = vec![0; n];
    for i in k..n {
        temp_outer.push(p[i]);
        init_vector[p[i]] += 1;
    }
    let mut lazy_outer1 = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);

    let inv_num = get_inv_num(&p);
    let inv_num_outer = get_inv_num(&temp_outer);
    let mut inv_num_internal = inv_num - inv_num_inner - inv_num_outer;
    ...
    let mut lazy_inner = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    let mut init_vector = vec![0; n];
    let mut lazy_outer0 = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);

    let mut inv_num_temp = (inv_num_outer + inv_num_internal) % modulus;
    top += factorial * inv_num_temp % modulus;
    top %= modulus;
    // println!("inv_num_temp = {:?}", inv_num_temp);
    for i in 1..num_range {
        // println!("i = {:?} ---- ---- ----", i);
        // シャッフル開始位置
        // インターナルと、アウターを逐次更新していけば良さそう

        // p[i-1] を抜けさせる
        lazy_inner.range_update(p[i-1], p[i-1], -1);
        // p[i-1] が抜ける分について考える
        let inc_internal = if p[i-1] != 0 {
            lazy_inner.range_query(0, p[i-1]-1).value as usize
        } else {
            0
        };
        // p[i+k-1] が加入する分について考える
        let dec_internal = if p[i+k-1] != n - 1 {
            lazy_inner.range_query(p[i+k-1] + 1, n-1).value as usize
        }
        else {
            0
        };
        // p[i+k-1] を加入させる
        lazy_inner.range_update(p[i+k-1], p[i+k-1], 1);

        inv_num_temp = (modulus + inv_num_temp + inc_internal - dec_internal) % modulus;
        // println!("inv_num_temp = {:?}", inv_num_temp);
    ...
    let ans = mod_dev(top, bottom, modulus);
    println!("{}", ans);
}
