...
fn main() {
    ...
    // 
    // 【Mo's algorithm】
    // 概要：長さN配列に対して，Q個の区間クエリを合計O(Q √N)で計算可能。
    // 条件：全クエリを先読みできる(オフラインクエリ)ことが適用条件。
    // 処理：アルゴリズムの内容的には、
    // (1) L,Rの2次元空間上に各クエリをマッピングする
    // (2) クエリiからクエリjへの移動時の計算量は、マンハッタン距離 O(|Li - Lj| + |Ri - Rj|) で決まることを理解する。
    // (2) L(横軸)座標を、√Q 個 (クエリ数の平方根) の区間に分割する。
    // (4) 左から順に全区間のクエリを実行していく。各区間のクエリは、下から上に移動していく。
    // (5) R(縦軸)の移動は、1区間で最大 N で済み、√Q 個の区間があるから、全クエリでO(N√Q)となる。
    // (6) L(横軸)の移動は、1回の移動で最大 N / √Q (1区間の横幅)で、それが全クエリでQ回あるから合計O(N√Q)となる。
    // (7) R,L (縦、横)の移動を合わせて、O(N√Q)となる。
    // 参考: https://kanpurin.hatenablog.com/entry/2021/04/09/001439

    // クエリQ, 区間N
    let num_section = (q as f64).sqrt().floor() as usize; // 区間の個数。小数点切り捨て
    let width = n / num_section + 1; // 区間の幅。n < num_section のとき、区間幅が0になって後でゼロ除算してしまうので、+1している。
    let mut section = vec![vec![]; num_section];
    for i in 0..q {
        let li = lr[i][0];
        let ri = lr[i][1];
        let index = li / width;
        section[index].push([ri, li, i]);
    }
    ...
    for i in 0..num_section {
        ...
        let iterator = if is_upward == 1 {
            (0..section[i].len()).collect::<Vec<usize>>()
        }
        else {
            (0..section[i].len()).rev().collect::<Vec<usize>>()
        };
        for j in iterator {
            let rij = section[i][j][0];
            let lij = section[i][j][1];
            let ind = section[i][j][2];
            if is_first_time {
                for k in lij..rij+1 {
                    hash[c[k]] += 1;
                    if hash[c[k]] == 1 {
                ...
                if lij < l_pre {
                    if rij < l_pre {
                        ...
                        // 減らす分
                        for k in rij+1..r_pre+1{
                            hash[c[k]] -= 1;
                            if hash[c[k]] == 0 {
                                count -= 1;
                            }
                            // hash.subtract_value_by_1(c[k]);
                        }
                    }
                    else {
                        // 増やす分
    ...
    // /// s1: 右の子data (たぶん)
    // /// 返り値: 2子から得られたdata
    // fn op(s0: &HashSet<isize>, s1:  &HashSet<isize>) -> HashSet<isize> {
    //     let mut s = s0.clone();
    //     for v in s1 {
    //         s.insert(*v);
    //     }
    ...
    // /// x: 更新前のdata
    // /// 返り値: 更新後のdata
    // fn mapping(f: &HashSet<isize> ,x: &HashSet<isize>) -> HashSet<isize> {
    //     let mut s =  x.clone();
    //     if let Some(v) = f.iter().next() {
    //         s.insert(*v);
    //     }
    //     return s
    // }
    ...
    // fn composition(f: &HashSet<isize> ,g: &HashSet<isize>) -> HashSet<isize> {
    //     let mut s = f.clone();
    //     for v in g {
    //         s.insert(*v);
    //     }
    ...
    //     n,
    //     e as fn() -> HashSet<isize>, 
    //     op as fn(&HashSet<isize>, &HashSet<isize>) -> HashSet<isize>, 
    //     mapping as fn(&HashSet<isize>, &HashSet<isize>) -> HashSet<isize>, 
    //     composition as fn(&HashSet<isize>, &HashSet<isize>) -> HashSet<isize>, 
    //     id as fn() -> HashSet<isize>
    // );

    // // lazy_segment_tree.print_all_single_queries();
    // for i in 0..n {
    //     let mut v = HashSet::new();
    //     v.insert(c[i] as isize);
    //     lazy_segment_tree.range_update(i, i, v);
    // }
    // // lazy_segment_tree.print_all_single_queries();


    // for i in 0..q {
    //     let v = lazy_segment_tree.range_query(l[i], r[i]);
    //     println!("{}", v.len());
    // }
...
impl<T: std::cmp::Ord +  std::hash::Hash> ValueManipulator for HashMap<T, usize> {
