...
fn main() {
    // 2025-07-18 20:36-21:17 (41min) <- 7/18から時間空いて忘れているので、ノーカウント
    // 2025-09-12 19:41-21:00 (1h19min)
    // 2025-09-13 10:32-11:35 (1h3min)
    // Total: (2h22min)
    input! {
        n: usize,
        a: [usize; n],
    ...
    // 考察
    // ΣΣ系の問題は、2次元の表を書いて、Σdp[L] に落とし込む方法を考えると良い。
    // そのためにL->L+1の差分や、寄与を色々考える必要がある。
    // 
    // 入力例2
    // N=4
    // A = [3  1   4   2   4]
    ...
    // L=3 _   _   _   1   2
    // L=4 _   _   _   _   1
    // f(L-1,R)とf(L,R)の差分を考える。
    // 具体的に、
    // f(0,R) - f(1,R) を考える。
    //     R=0 R=1 R=2 R=3 R=4
    // L=1 _   -1  +0  +1  +1
    // なんだか単調性がありそう。基本的に、f(L-1,R)とf(L,R)の差は、a[0]の有無である。
    // a[0] ± 1 の範囲だけ影響がある。
    // [1]-1になるケース ... [L,R]の範囲内にa[0]-1, a[0], a[0]+1  が存在しない。
    // [2]+0になるケース ... [L,R]の範囲内にa[0]-1, a[0], a[0]+1  のいずれかが存在し始める最小のR。
    // [3]+1になるケース ... [L,R]の範囲内にa[0] が存在しない　かつ　a[0]-1, a[0]+1  が存在し始める最小のR。

    let mut num_to_poss = vec![BTreeSet::new(); n+2];
    for i in 0..n {
        num_to_poss[a[i]].insert(i);
    }
    ...
    for r in 0..n {
        ...
        else if set.contains(&(ar-1)) && set.contains(&(ar+1)) {
            count = count - 1;
        }
        else if set.contains(&(ar-1)) || set.contains(&(ar+1)) {
            // 変化なし
    ...
    for l in 1..n {
        ...
        let mut ind_c = n;
        let mut ind_p = n;
        if let Some(&ind) = num_to_poss[a_n].iter().next() {
            ind0 = ind.min(ind0);
            ind_n = ind;
        }
        if let Some(&ind) = num_to_poss[a_c].iter().next() {
            ind0 = ind.min(ind0);
            ind_c = ind;
        }
        if let Some(&ind) = num_to_poss[a_p].iter().next() {
            ind0 = ind.min(ind0);
            ind_p = ind;
        }

        let ind1 = max(ind_p,ind_n);
        if ind0 != l {
            lazy_segment_tree.range_update(l, ind0-1, -1);
        }
        if ind1 <= ind_c-1 {
            lazy_segment_tree.range_update(ind1, ind_c-1, 1);
        } 
        let cont = lazy_segment_tree.range_query(l, n-1).value;
        ans += cont;
        num_to_poss[a[l]].remove(&l);

        // ind0 ... はじめてa[l-1]-1, a[l-1], a[l-1]+1 となる要素が現れるとき
        // ind1 ... 

        // left..ind0 は、count = count - 1 で済む。
        // ind0..ind1 は、count = count + 0 となる。
        // ind1..n は、count = count + 1 となる。
    }
