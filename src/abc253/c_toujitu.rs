use proconio::input;
use itertools::Itertools;
use std::cmp::{max, min};
use std::cmp::Ordering;
use std::collections::VecDeque;
use std::collections::BinaryHeap;
use std::collections::HashMap;
use proconio::marker::Chars;

// BinaryHeapの根を最大値ではなく最小値にするために構造体を書き換える
#[derive(Copy, Clone, Eq, PartialEq)]
struct MinHeapVertex {
    value: usize,
    // position: usize,
}

impl MinHeapVertex {
    fn new(value: usize) -> Self {
        MinHeapVertex { value: value }
    }
}

// The priority queue depends on `Ord`.
// Explicitly implement the trait so the queue becomes a min-heap
// instead of a max-heap.
// impl トレイト名 for 構造体名
impl Ord for MinHeapVertex {
    fn cmp(&self, other: &Self) -> Ordering {
        other.value.cmp(&self.value)
        // &self.value.cmp(other.value) // デフォルトのMax-Heapはselfとotherが置換されている
    }
}

// `PartialOrd` needs to be implemented as well.
// impl トレイト名 for 構造体名
impl PartialOrd for MinHeapVertex {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

fn main() {
    input! {q: usize}
    let mut query = vec![];

    for _ in 0..q {
        input! {n: usize}
        match n {
            1 => {  input! {x: usize};
                    query.push(vec![n, x])
                },
            2 => {  input! {x: usize, c: usize};
                    query.push(vec![n, x, c])
                },
            3 => (  query.push(vec![n])
                ),
            _ => (),
        }
    }

    let mut hash_map = HashMap::new();
    let mut heap_min = BinaryHeap::new();
    let mut heap_max = BinaryHeap::new();
    
    // let mut s_max = 0;
    // let mut s_min = 10_usize.pow(9_u32);
    for i in query {
        match i[0] {
            1 => {  // 1 x : S に x を 1 個追加する。
                    if hash_map.contains_key(&i[1]) {
                        // 既にhash_mapに存在するなら+1
                        *hash_map.get_mut(&i[1]).unwrap() += 1;
                    }
                    else {
                        // hash_mapに存在しなければ新規に追加
                        hash_map.insert(i[1], 1);
                    }
                    // 既にhash_mapに存在していれば、そのまま終了
                    if hash_map[&i[1]] >= 2 {
                        continue
                    }
                    // 0->1個に増えるとき、heapに追加する 
                    else {
                        // heap_min.push(MinHeapVertex::new(i[1]));
                        heap_min.push(- (i[1] as isize));
                        heap_max.push(i[1] as isize);
                    }
                },
            2 => {  // 2 x c : S から x を min(c, (S に含まれる x の個数 )) 個削除する。
                    if !(hash_map.contains_key(&i[1])) {
                        // hash_mapに存在しないなら無視
                        continue
                    }
                    // S(=hash_map)からxを何個か削除
                    *hash_map.get_mut(&i[1]).unwrap() -= min(i[2], hash_map[&i[1]]);
                    
                    // S(=hash_map)で0個になってしまったら、heapの頂点から削除
                    if hash_map[&i[1]] == 0 {
                        // if heap_min.peek().unwrap().value == i[1] {
                        if *(heap_min.peek().unwrap()) == - (i[1] as isize) {
                            heap_min.pop();
                        }
                        if *(heap_max.peek().unwrap())== i[1] as isize{
                            heap_max.pop();
                        }
                    }
                },
            // 3 => {println!("{}", *(heap_max.peek().unwrap()) - heap_min.peek().unwrap().value);},
            3 => {println!("{}", *(heap_max.peek().unwrap()) + *(heap_min.peek().unwrap()));},
            _ => (),
        }
        // println!("s: {:?}", s);
        // println!("{:?}", i);
    }
}