...
use rand::Rng;
fn main() {
    // 2024-10-29 12:31-12:50 (19min)
    // 2024-10-29 18:36-19:16 (40min)
    // Total: 59min
    input! {
        n: usize, // 2 <= n, m <= 10^5
        m: usize,
        x: [isize; n], // -10^9 <= x,y <= 10^9
        y: [isize; m],
    }

    // 考察
    // 簡単のために、線と線の区間の長さを1とする。
    // 例えば、xの一番左の長さ1の区間[0,1]を底辺(長さdx[0,1]=1)とする長方形の面積の和S[0,1]を考えると、、
    // S[0,1] = dx[0,1] * (長さ1の縦辺の和 + 長さ2の縦辺の和 +, ... + 長さm-1の縦辺の和)
    // となる。
    // 求める面積Sは、
    // S = Σ S[i,j] (i<j) = Σ dx[i,j] * (長さ1の縦辺の和 + 長さ2の縦辺の和 +, ... + 長さm-1の縦辺の和) (i<j)
    //   = (長さ1の縦辺の和 + 長さ2の縦辺の和 +, ... + 長さm-1の縦辺の和) * Σ dx[i,j] (i<j)
    //   = (長さ1の縦辺の和 + 長さ2の縦辺の和 +, ... + 長さm-1の縦辺の和) * (長さ1の底辺の和 + 長さ2の底辺の和 +, ... + 長さm-1の底辺の和)
    // よって、縦辺と、横辺の和をそれぞれ求めれば良い。
    // 
    // 例えば、n=6の場合の底辺の和を求めることを考える。

    // 各区間における、長さLの区間が通る回数を * の個数で対応させると以下の図のようになる。
    ...
    // よって、この * の数を足し合わせれば、O(n) で、底辺の数の和を数える事ができる。
    // 縦辺の数の和も同様に求められる。
    ...
    // 遅延評価セグメント木 + Modint
    // ※モジュール内が、modulus = 1_000_000_007 　で、ハードコーディングされているので注意。適宜書き換えが要る。

    let mi_generator = modint::ModintGenerator::new(modulus);
    let zero = mi_generator.generate(0);

    // lazy_x[i] := xのi番目の区間の登場頻度
    let mut init_vector = vec![zero; n-1];
    let mut lazy_x = lazy_segment_tree::mod_new_range_increment_update_and_range_sum_query_from_vec(&init_vector);

    // lazy_y[i] := yのi番目の区間の登場頻度
    let mut init_vector = vec![zero; m-1];
    let mut lazy_y = lazy_segment_tree::mod_new_range_increment_update_and_range_sum_query_from_vec(&init_vector);
    ...
    for i in 0..n/2 {
        ...
        let update_l = i;
        let update_r = n-2-i;
        // L = i+1 と、L = n-1-2i の底辺の寄与を同時に反映
        lazy_x.range_update(update_l, update_r, mi_generator.generate(count as isize));
        // println!("update_l = {}, update_r = {:?}", update_l, update_r);
        // println!("count = {:?}", count);
    }
    ...
    for i in 0..m/2 {
        ...
        let update_l = i;
        let update_r = m-2-i;
        // L = i+1 と、L = m-1-2i の縦辺の寄与を同時に反映
        // println!("update_l = {}, update_r = {:?}", update_l, update_r);
        lazy_y.range_update(update_l, update_r, mi_generator.generate(count as isize));
    }

    // println!("lazy_x");
    // lazy_x.print_all_single_queries();
    // println!("lazy_y");
    // lazy_y.print_all_single_queries();

    let mut sum_x = 0;
    let mut sum_y = 0;

    for i in 0..n-1 {
        let dx = (x[i+1] - x[i]) % modulus;
        let num = lazy_x.range_query(i, i).value.value;
        sum_x += dx * num;
        sum_x %= modulus;
    }
    for i in 0..m-1 {
        let dy: isize = (y[i+1] - y[i]) % modulus; 
        let num = lazy_y.range_query(i, i).value.value;
        sum_y += dy * num;
        sum_y %= modulus;
    }
    let ans = sum_x * sum_y % modulus;
    println!("{}", ans);
...
pub mod lazy_segment_tree {
    ...
    pub fn mod_new_range_increment_update_and_range_sum_query(list_size: usize) -> LazySegmentTree<SSumMod, fn(&SSumMod, &SSumMod) -> SSumMod, fn() -> SSumMod, Modint, fn(&Modint, &SSumMod) -> SSumMod, fn(&Modint, &Modint) -> Modint, fn() -> Modint> {    
        ...
        fn e() -> SSumMod {
            let init_value: Modint = Modint{modulus: 1_000_000_007, value: 0};
            let init_size: isize = 0;
