...
use superslice::*;
fn main() {
    // 2025-06-21 17:33-18:06 (33min)
    // BITで、bit毎に計算したが、実はセグ木にそのまま載せられる。
    // xor演算子 は結合律を満たすし、単位元が存在する(0)ので、そのままセグ木に載せられる。
    input! {
        n: usize,
        q: usize,
        mut a: [usize; n],
    ...
    // let mut init_vector = vec![0; n+1];
    // let mut lazys = vec![lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector); 31];
    let mut bitrees = vec![BinaryIndexedTree::new(n+1); 31];
    for i in 1..=n {
        for b in 0..31 {
            if a[i] & (1 << b) != 0 {
                // lazys[b].range_update(i, i, 1);
                bitrees[b].add(i,  1);
            }
        }
    }
    ...
    for i in 0..q {
        ...
        // 3_00_000
            ...
            let mut ans  = 0;
            for b in 0..31 {
                // let num  = lazys[b].range_query(xi, yi).value;
                let num = bitrees[b].sum_range(xi,  yi);
                if num % 2 != 0 {
                    ans += (1 << b);
                }
            }
