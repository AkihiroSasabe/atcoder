...
fn main() {
    ...
    for i in 0..n {
        ...
        *set.entry(xi).or_insert(0) += hi;
    }
    // 必要な爆撃回数
    let mut x = vec![];
    let mut init_vector = vec![];
    for (xi, hi) in set.iter() {
        let mut num = hi / a;
        if hi % a != 0 {
            num += 1;
        }
        init_vector.push(num as isize);
        x.push(*xi);
    }
    // println!("set = {:?}", set);
    let mut ans = 0;
    ...
    for (i, (xi, hi)) in set.iter().enumerate() {
        let num = lazy_segment_tree.range_query(i, i).value;
        if num <= 0 {continue}
        // xi + 2*d以下で、最大のindexを知りたい。
        let right_ind = x.upper_bound(&(*xi + 2*d)) - 1;
        lazy_segment_tree.range_update(i, right_ind, -num);
        ans += num;
    }
    println!("{}", ans);
