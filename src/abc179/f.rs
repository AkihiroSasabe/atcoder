...
fn main() {
    ...
    // 2025-05-22 19:41-21:15 (1h34min)
    input! {
        n: usize,
        q: usize,
    ...
    let init_vector = vec![(n-1) as isize; n];

    // lazy_y.range_query(y,y) := 列y において、一番上にある白マスの位置
    let mut lazy_y = lazy_segment_tree::new_range_assignment_update_and_range_minimum_query_from_vec(&init_vector);
    
    // lazy_x.range_query(x,x) := 列x において、一番上にある白マスの位置
    let mut lazy_x = lazy_segment_tree::new_range_assignment_update_and_range_minimum_query_from_vec(&init_vector);
    
    // 初期化
    // 一番最後の列と行は、先頭から白マス
    lazy_x.range_update(n-1, n-1, 0);
    lazy_y.range_update(n-1, n-1, 0);
    
    // 先頭の列と行も、先頭から白マス
    lazy_x.range_update(0, 0, 0);
    lazy_y.range_update(0, 0, 0);
    ...
    for i in 0..q {
        ...
        if ki == 1 {
            ...
            // 取得 (xにおける白の一番上)
            let highest = lazy_x.range_query(x, x) as usize;
            // 黒石から白石になる個数を計算
            let diff = highest - 1;
            ans -= diff;

            // println!("(i, x, highest, diff, ans) = {:?} **** **** ", (i, x, highest, diff, ans));

            // (1,x)に白石を置いた時、x列目は縦に白石となるが、それが行yにとって一番左にある白石となり得るか?
            // 実は、yi < yj のとき 行yiにとって一番左にある白石であるとき、yjも一番左にある白石であることが実験で分かる。
            // よって行yiにとっては一番左の白石けど、yi-1にとっては一番左の白石じゃない境界を2分探索していけばよい。
            
            // まず一番下で塗り替わる白石が、一番左にある白石になり得るかをチェック！
            if (x as isize) < lazy_y.range_query(highest-1, highest-1) {
                // 二分探索
                let mut ok = highest - 1;
                let mut ng = 0;
                while (ok - ng) > 1 {
                    let mid = (ok + ng) / 2;
                    // (y=mid, x=x)の白石が、x行目の一番左にいる白石になり得るか?
                    let is_ok = (x as isize) < lazy_y.range_query(mid, mid);
                    if is_ok {
                        ok = mid;
                    }
                    else {
                ...
                // 更新 (一番左にある白石)
                lazy_y.range_update(ok, highest-1, x as isize);
            }
            lazy_x.range_update(x, x, 0 as isize);
        }
        else {
            // (y,1) に白石を置く。
            let y = pos;
            ...
            // 取得 (yにおける白の一番左)
            let leftest = lazy_y.range_query(y, y) as usize;

            let diff = leftest - 1;
            ans -= diff;
            // println!("(i, y, leftest, diff, ans) = {:?} **** **** ", (i, y, leftest, diff, ans));

            // 更新
            if (y as isize) < lazy_x.range_query(leftest-1, leftest-1) {
                // にぶたん
                let mut ok = leftest - 1;
                let mut ng = 0;
                while (ok - ng) > 1 {
                    let mid = (ok + ng) / 2;
                    let is_ok = (y as isize) < lazy_x.range_query(mid, mid);
                    if is_ok {
                        ok = mid;
                    }
                    else {
                ...
                // 更新
                lazy_x.range_update(ok, leftest-1, y as isize);
            }
            lazy_y.range_update(y, y, 0 as isize);
        }
        // println!("i = {i}, ans = {:?}", ans);
        // print!("lazy_x: ");
        // lazy_x.print_list();
