...
fn main() {
    ...
    let init_vector = a.clone();
    let mut lazy_segment_tree = lazy_segment_tree::new_range_increment_update_and_range_sum_query_from_vec(&init_vector);

    for i in 0..m {
        let bi = b[i];
        let num = lazy_segment_tree.range_query(bi, bi).value as usize;
        lazy_segment_tree.range_update(bi, bi, - (num as isize));

        let cycles = num / n;
        lazy_segment_tree.range_update(0, n-1, cycles as isize);
        
        let remain = num % n;
        let next = bi + remain;

        if next < n {
            lazy_segment_tree.range_update(bi + 1, next, 1);
        }
        else {
            let next = next % n;
            if bi + 1 == n {
                lazy_segment_tree.range_update(0, next, 1);
            }
            else {
                lazy_segment_tree.range_update(bi+1, n-1, 1);
                lazy_segment_tree.range_update(0, next, 1);
            }
        }
    }
    ...
    for i in 0..n {
        let num = lazy_segment_tree.range_query(i, i).value as usize;
        print!("{} ", num);
    }
