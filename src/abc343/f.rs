...
use superslice::*;
fn main() {
    // 2024-03-03 10:14-11:55 (1h41min)
    input! {
        n: usize,
        q: usize,
        mut a: [usize; n],
    ...
    // 単位元はある? -> ある。maxを取るので、必ず a[i] よりも小さい0 が、単位元となる。
    // op(s, e) = 
    // 結合律はある? -> ある。どの順番にopしても、最大値と、準最大値の値は変わらないし、その個数も変わらない。
    ...
    fn op(&sl: &SX, &sr:  &SX) -> SX {
        let mut btree = BTreeMap::new();
        *btree.entry(sl.max0).or_insert(0) += sl.num0;
        *btree.entry(sl.max1).or_insert(0) += sl.num1;
        *btree.entry(sr.max0).or_insert(0) += sr.num0;
        *btree.entry(sr.max1).or_insert(0) += sr.num1;
        
        let mut iter = btree.iter().rev();
        let (&max0, _) = iter.next().unwrap();
        let (&max1, _) = iter.next().unwrap();

        let num0 = *btree.get(&max0).unwrap();
        let num1 = *btree.get(&max1).unwrap();
    ...
    let mut lst = lazy_segment_tree::LazySegmentTree::new(
        n,
        e as fn() -> SX, 
        op as fn(&SX, &SX) -> SX, 
        mapping as fn(&SX, &SX) -> SX, 
        composition as fn(&SX, &SX) -> SX, 
        id as fn() -> SX
    );
    ...
    for i in 0..q {
        if k[i] == 1 {
            ...
            lst.range_update(p[i] - 1, p[i] - 1, sx);
        }
        else if k[i] == 2 {
            let sx = lst.range_query(p[i] - 1, x[i] - 1);
            println!("{}", sx.num1);
        }
    }
